...
$cfg_make_companies_add = NULL; // Initialize page object first

class ccfg_make_companies_add extends ccfg_make_companies {
	...
	// Page object name
	var $PageObjName = 'cfg_make_companies_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "cfg_make_companiesview.php")
	...
	function LoadDefaultValues() {
		...
		$this->logo_link->Upload->DbValue = NULL;
		$this->logo_link->OldValue = $this->logo_link->Upload->DbValue;
		$this->logo_link->CurrentValue = NULL; // Clear file related field
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->logo_link);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->logo_link->Visible && !$this->logo_link->Upload->KeepFile) {
			$this->logo_link->Upload->DbValue = ""; // No need to delete old file
...
// Create page object
if (!isset($cfg_make_companies_add)) $cfg_make_companies_add = new ccfg_make_companies_add();

// Page init
$cfg_make_companies_add->Page_Init();

// Page main
$cfg_make_companies_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_make_companies_add->Page_Render();
?>
...
var CurrentForm = fcfg_make_companiesadd = new ew_Form("fcfg_make_companiesadd", "add");

// Validate form
fcfg_make_companiesadd.Validate = function() {
...
fcfg_make_companiesadd.Form_CustomValidate = 
...
fcfg_make_companiesadd.ValidateRequired = true;
<?php } else { ?>
fcfg_make_companiesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_make_companiesadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_make_companiesadd.Lists["x_status"].Options = <?php echo json_encode($cfg_make_companies->status->Options()) ?>;
...
<?php $cfg_make_companies_add->ShowPageHeader(); ?>
<?php
$cfg_make_companies_add->ShowMessage();
?>
<form name="fcfg_make_companiesadd" id="fcfg_make_companiesadd" class="<?php echo $cfg_make_companies_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_make_companies_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_make_companies_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_make_companies_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fcfg_make_companiesadd.Init();
</script>
<?php
$cfg_make_companies_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_make_companies_add->Page_Terminate();
?>
