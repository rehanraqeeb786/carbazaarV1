...
$cfg_body_type_search = NULL; // Initialize page object first

class ccfg_body_type_search extends ccfg_body_type {
	...
	// Page object name
	var $PageObjName = 'cfg_body_type_search';
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "cfg_body_typelist.php" . "?" . $sSrchStr;
						if ($this->IsModal) {
							$row = array();
							$row["url"] = $sSrchStr;
...
// Create page object
if (!isset($cfg_body_type_search)) $cfg_body_type_search = new ccfg_body_type_search();

// Page init
$cfg_body_type_search->Page_Init();

// Page main
$cfg_body_type_search->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_body_type_search->Page_Render();
?>
...
<?php if ($cfg_body_type_search->IsModal) { ?>
var CurrentAdvancedSearchForm = fcfg_body_typesearch = new ew_Form("fcfg_body_typesearch", "search");
<?php } else { ?>
var CurrentForm = fcfg_body_typesearch = new ew_Form("fcfg_body_typesearch", "search");
<?php } ?>

// Form_CustomValidate event
fcfg_body_typesearch.Form_CustomValidate = 
...
fcfg_body_typesearch.ValidateRequired = true;
<?php } else { ?>
fcfg_body_typesearch.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_body_typesearch.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_body_typesearch.Lists["x_status"].Options = <?php echo json_encode($cfg_body_type->status->Options()) ?>;

// Form object for search
// Validate function for search

fcfg_body_typesearch.Validate = function(fobj) {
...
<?php } ?>
<?php $cfg_body_type_search->ShowPageHeader(); ?>
<?php
$cfg_body_type_search->ShowMessage();
?>
<form name="fcfg_body_typesearch" id="fcfg_body_typesearch" class="<?php echo $cfg_body_type_search->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_body_type_search->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_body_type_search->Token ?>">
<?php } ?>
...
<?php if ($cfg_body_type->title->Visible) { // title ?>
	<div id="r_title" class="form-group">
		<label for="x_title" class="<?php echo $cfg_body_type_search->SearchLabelClass ?>"><span id="elh_cfg_body_type_title"><?php echo $cfg_body_type->title->FldCaption() ?></span>	
		...
		<div class="<?php echo $cfg_body_type_search->SearchRightColumnClass ?>"><div<?php echo $cfg_body_type->title->CellAttributes() ?>>
...
<?php if ($cfg_body_type->details->Visible) { // details ?>
	<div id="r_details" class="form-group">
		<label for="x_details" class="<?php echo $cfg_body_type_search->SearchLabelClass ?>"><span id="elh_cfg_body_type_details"><?php echo $cfg_body_type->details->FldCaption() ?></span>	
		...
		<div class="<?php echo $cfg_body_type_search->SearchRightColumnClass ?>"><div<?php echo $cfg_body_type->details->CellAttributes() ?>>
			<span id="el_cfg_body_type_details">
<input type="text" data-table="cfg_body_type" data-field="x_details" name="x_details" id="x_details" size="35" placeholder="<?php echo ew_HtmlEncode($cfg_body_type->details->getPlaceHolder()) ?>" value="<?php echo $cfg_body_type->details->EditValue ?>"<?php echo $cfg_body_type->details->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($cfg_body_type->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label class="<?php echo $cfg_body_type_search->SearchLabelClass ?>"><span id="elh_cfg_body_type_status"><?php echo $cfg_body_type->status->FldCaption() ?></span>	
		...
		<div class="<?php echo $cfg_body_type_search->SearchRightColumnClass ?>"><div<?php echo $cfg_body_type->status->CellAttributes() ?>>
...
if (is_array($arwrk)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($cfg_body_type->status->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " checked" : "";
		if ($selwrk <> "")
			$emptywrk = FALSE;
?>
...
<script type="text/javascript">
fcfg_body_typesearch.Init();
</script>
<?php
$cfg_body_type_search->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_body_type_search->Page_Terminate();
?>
