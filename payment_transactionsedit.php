...
$payment_transactions_edit = NULL; // Initialize page object first

class cpayment_transactions_edit extends cpayment_transactions {
	...
	// Page object name
	var $PageObjName = 'payment_transactions_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("payment_transactionslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// ad_id
			$this->ad_id->SetDbValueDef($rsnew, $this->ad_id->CurrentValue, 0, $this->ad_id->ReadOnly);

			// package_id
			$this->package_id->SetDbValueDef($rsnew, $this->package_id->CurrentValue, 0, $this->package_id->ReadOnly);
			...
			// order_reference_id
			$this->order_reference_id->SetDbValueDef($rsnew, $this->order_reference_id->CurrentValue, "", $this->order_reference_id->ReadOnly);

			// amount
			$this->amount->SetDbValueDef($rsnew, $this->amount->CurrentValue, 0, $this->amount->ReadOnly);

			// response
			$this->response->SetDbValueDef($rsnew, $this->response->CurrentValue, NULL, $this->response->ReadOnly);
...
// Create page object
if (!isset($payment_transactions_edit)) $payment_transactions_edit = new cpayment_transactions_edit();

// Page init
$payment_transactions_edit->Page_Init();

// Page main
$payment_transactions_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$payment_transactions_edit->Page_Render();
?>
...
var CurrentForm = fpayment_transactionsedit = new ew_Form("fpayment_transactionsedit", "edit");

// Validate form
fpayment_transactionsedit.Validate = function() {
...
fpayment_transactionsedit.Form_CustomValidate = 
...
fpayment_transactionsedit.ValidateRequired = true;
<?php } else { ?>
fpayment_transactionsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpayment_transactionsedit.Lists["x_ad_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_ad_title","x_demand_price","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpayment_transactionsedit.Lists["x_package_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_package_fee","x_number_of_days","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpayment_transactionsedit.Lists["x_pay_method_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpayment_transactionsedit.Lists["x_bank_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_bank_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpayment_transactionsedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpayment_transactionsedit.Lists["x_status"].Options = <?php echo json_encode($payment_transactions->status->Options()) ?>;
...
<?php $payment_transactions_edit->ShowPageHeader(); ?>
<?php
$payment_transactions_edit->ShowMessage();
?>
<form name="fpayment_transactionsedit" id="fpayment_transactionsedit" class="<?php echo $payment_transactions_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($payment_transactions_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $payment_transactions_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fpayment_transactionsedit.CreateAutoSuggest({"id":"x_ad_id","forceSelect":false});
</script>
</span>
...
<input type="hidden" data-table="payment_transactions" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($payment_transactions->id->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $payment_transactions_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fpayment_transactionsedit.Init();
</script>
<?php
$payment_transactions_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$payment_transactions_edit->Page_Terminate();
?>
