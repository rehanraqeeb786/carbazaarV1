...
$ad_features_edit = NULL; // Initialize page object first

class cad_features_edit extends cad_features {
	...
	// Page object name
	var $PageObjName = 'ad_features_edit';
	...
	function Page_Main() {
		...
		if ($this->ID->CurrentValue == "")
			$this->Page_Terminate("ad_featureslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($this->feature_id->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`feature_id` = " . ew_AdjustSql($this->feature_id->CurrentValue, $this->DBID) . ")";
		...
		if ($rs->EOF) {
			...
			// feature_id
			$this->feature_id->SetDbValueDef($rsnew, $this->feature_id->CurrentValue, 0, $this->feature_id->ReadOnly);
...
// Create page object
if (!isset($ad_features_edit)) $ad_features_edit = new cad_features_edit();

// Page init
$ad_features_edit->Page_Init();

// Page main
$ad_features_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ad_features_edit->Page_Render();
?>
...
var CurrentForm = fad_featuresedit = new ew_Form("fad_featuresedit", "edit");

// Validate form
fad_featuresedit.Validate = function() {
...
fad_featuresedit.Form_CustomValidate = 
...
fad_featuresedit.ValidateRequired = true;
<?php } else { ?>
fad_featuresedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fad_featuresedit.Lists["x_ad_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_ad_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fad_featuresedit.Lists["x_feature_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_feature_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $ad_features_edit->ShowPageHeader(); ?>
<?php
$ad_features_edit->ShowMessage();
?>
<form name="fad_featuresedit" id="fad_featuresedit" class="<?php echo $ad_features_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ad_features_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ad_features_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fad_featuresedit.CreateAutoSuggest({"id":"x_ad_id","forceSelect":false});
</script>
</span>
<?php } ?>
...
<input type="hidden" data-table="ad_features" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($ad_features->ID->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ad_features_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fad_featuresedit.Init();
</script>
<?php
$ad_features_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ad_features_edit->Page_Terminate();
?>
