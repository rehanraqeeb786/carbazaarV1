...
$cfg_features_add = NULL; // Initialize page object first

class ccfg_features_add extends ccfg_features {
	...
	// Page object name
	var $PageObjName = 'cfg_features_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "cfg_featuresview.php")
	...
	function LoadDefaultValues() {
		$this->feature_name->CurrentValue = NULL;
		$this->feature_name->OldValue = $this->feature_name->CurrentValue;
		$this->feature_icon_link->Upload->DbValue = NULL;
		$this->feature_icon_link->OldValue = $this->feature_icon_link->Upload->DbValue;
		$this->feature_icon_link->CurrentValue = NULL; // Clear file related field
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->feature_icon_link);
	...
	function AddRow($rsold = NULL) {
		...
		// feature_name
		$this->feature_name->SetDbValueDef($rsnew, $this->feature_name->CurrentValue, "", FALSE);

		// feature_icon_link
		if ($this->feature_icon_link->Visible && !$this->feature_icon_link->Upload->KeepFile) {
			$this->feature_icon_link->Upload->DbValue = ""; // No need to delete old file
...
// Create page object
if (!isset($cfg_features_add)) $cfg_features_add = new ccfg_features_add();

// Page init
$cfg_features_add->Page_Init();

// Page main
$cfg_features_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_features_add->Page_Render();
?>
...
var CurrentForm = fcfg_featuresadd = new ew_Form("fcfg_featuresadd", "add");

// Validate form
fcfg_featuresadd.Validate = function() {
...
fcfg_featuresadd.Form_CustomValidate = 
...
fcfg_featuresadd.ValidateRequired = true;
<?php } else { ?>
fcfg_featuresadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_featuresadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_featuresadd.Lists["x_status"].Options = <?php echo json_encode($cfg_features->status->Options()) ?>;
...
<?php $cfg_features_add->ShowPageHeader(); ?>
<?php
$cfg_features_add->ShowMessage();
?>
<form name="fcfg_featuresadd" id="fcfg_featuresadd" class="<?php echo $cfg_features_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_features_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_features_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_features_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fcfg_featuresadd.Init();
</script>
<?php
$cfg_features_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_features_add->Page_Terminate();
?>
