...
//
// Table class for bank_accounts
//
class cbank_accounts extends cTable {
	var $id;
	var $bank_name;
	var $logo;
	var $bank_account_title;
	var $bank_account;
	var $branch_code;
	var $status;
	var $created_at;
	...
	function __construct() {
		...
		$this->TableVar = 'bank_accounts';
		$this->TableName = 'bank_accounts';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`bank_accounts`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('bank_accounts', 'bank_accounts', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// bank_name
		$this->bank_name = new cField('bank_accounts', 'bank_accounts', 'x_bank_name', 'bank_name', '`bank_name`', '`bank_name`', 200, -1, FALSE, '`bank_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['bank_name'] = &$this->bank_name;

		// logo
		$this->logo = new cField('bank_accounts', 'bank_accounts', 'x_logo', 'logo', '`logo`', '`logo`', 200, -1, TRUE, '`logo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->fields['logo'] = &$this->logo;

		// bank_account_title
		$this->bank_account_title = new cField('bank_accounts', 'bank_accounts', 'x_bank_account_title', 'bank_account_title', '`bank_account_title`', '`bank_account_title`', 200, -1, FALSE, '`bank_account_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['bank_account_title'] = &$this->bank_account_title;

		// bank_account
		$this->bank_account = new cField('bank_accounts', 'bank_accounts', 'x_bank_account', 'bank_account', '`bank_account`', '`bank_account`', 200, -1, FALSE, '`bank_account`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['bank_account'] = &$this->bank_account;

		// branch_code
		$this->branch_code = new cField('bank_accounts', 'bank_accounts', 'x_branch_code', 'branch_code', '`branch_code`', '`branch_code`', 200, -1, FALSE, '`branch_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['branch_code'] = &$this->branch_code;

		// status
		$this->status = new cField('bank_accounts', 'bank_accounts', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->status->OptionCount = 2;
		$this->fields['status'] = &$this->status;

		// created_at
		$this->created_at = new cField('bank_accounts', 'bank_accounts', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('bank_accounts', 'bank_accounts', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`bank_accounts`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("bank_accountsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("bank_accountsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "bank_accountsadd.php?" . $this->UrlParm($parm);
		else
			$url = "bank_accountsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("bank_accountsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("bank_accountsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("bank_accountsdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->bank_account_title->EditValue = $this->bank_account_title->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->bank_account_title->Exportable) $Doc->ExportCaption($this->bank_account_title);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->bank_account_title->Exportable) $Doc->ExportField($this->bank_account_title);
