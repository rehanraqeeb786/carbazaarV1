...
$car_ads_edit = NULL; // Initialize page object first

class ccar_ads_edit extends ccar_ads {
	...
	function Page_Main() {
		...
		if ($this->ID->CurrentValue == "")
			$this->Page_Terminate("car_adslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->ad_title->FldIsDetailKey) {
			$this->ad_title->setFormValue($objForm->GetValue("x_ad_title"));
		}
	...
	function RestoreFormValues() {
		...
		$this->ad_title->CurrentValue = $this->ad_title->FormValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			$this->ad_title->EditValue = ew_HtmlEncode($this->ad_title->CurrentValue);
	...
	function ValidateForm() {
		...
		if (in_array("ad_features", $DetailTblVar) && $GLOBALS["ad_features"]->DetailEdit) {
		...
		if (in_array("ad_pictures", $DetailTblVar) && $GLOBALS["ad_pictures"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// ad_title
			$this->ad_title->SetDbValueDef($rsnew, $this->ad_title->CurrentValue, NULL, $this->ad_title->ReadOnly);

			// year_id
			$this->year_id->SetDbValueDef($rsnew, $this->year_id->CurrentValue, 0, $this->year_id->ReadOnly);

			// registered_in
			$this->registered_in->SetDbValueDef($rsnew, $this->registered_in->CurrentValue, 0, $this->registered_in->ReadOnly);
			...
			// model_id
			$this->model_id->SetDbValueDef($rsnew, $this->model_id->CurrentValue, NULL, $this->model_id->ReadOnly);

			// version_id
			$this->version_id->SetDbValueDef($rsnew, $this->version_id->CurrentValue, NULL, $this->version_id->ReadOnly);

			// milage
			$this->milage->SetDbValueDef($rsnew, $this->milage->CurrentValue, NULL, $this->milage->ReadOnly);

			// color_id
			$this->color_id->SetDbValueDef($rsnew, $this->color_id->CurrentValue, 0, $this->color_id->ReadOnly);

			// demand_price
			$this->demand_price->SetDbValueDef($rsnew, $this->demand_price->CurrentValue, 0, $this->demand_price->ReadOnly);

			// details
			$this->details->SetDbValueDef($rsnew, $this->details->CurrentValue, "", $this->details->ReadOnly);

			// engine_type_id
			$this->engine_type_id->SetDbValueDef($rsnew, $this->engine_type_id->CurrentValue, NULL, $this->engine_type_id->ReadOnly);

			// engine_capicity
			$this->engine_capicity->SetDbValueDef($rsnew, $this->engine_capicity->CurrentValue, NULL, $this->engine_capicity->ReadOnly);

			// transmition
			$this->transmition->SetDbValueDef($rsnew, $this->transmition->CurrentValue, NULL, $this->transmition->ReadOnly);

			// assembly
			$this->assembly->SetDbValueDef($rsnew, $this->assembly->CurrentValue, NULL, $this->assembly->ReadOnly);

			// mobile_number
			$this->mobile_number->SetDbValueDef($rsnew, $this->mobile_number->CurrentValue, "", $this->mobile_number->ReadOnly);

			// secondary_number
			$this->secondary_number->SetDbValueDef($rsnew, $this->secondary_number->CurrentValue, NULL, $this->secondary_number->ReadOnly);
			...
			// allow_whatsapp
			$this->allow_whatsapp->SetDbValueDef($rsnew, ((strval($this->allow_whatsapp->CurrentValue) == "1") ? "1" : "0"), 0, $this->allow_whatsapp->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("ad_features", $DetailTblVar) && $GLOBALS["ad_features"]->DetailEdit) {
						if (!isset($GLOBALS["ad_features_grid"])) $GLOBALS["ad_features_grid"] = new cad_features_grid(); // Get detail page object
						$EditRow = $GLOBALS["ad_features_grid"]->GridUpdate();
					}
				}
				if ($EditRow) {
					if (in_array("ad_pictures", $DetailTblVar) && $GLOBALS["ad_pictures"]->DetailEdit) {
						if (!isset($GLOBALS["ad_pictures_grid"])) $GLOBALS["ad_pictures_grid"] = new cad_pictures_grid(); // Get detail page object
						$EditRow = $GLOBALS["ad_pictures_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("ad_features", $DetailTblVar)) {
				...
				if ($GLOBALS["ad_features_grid"]->DetailEdit) {
					$GLOBALS["ad_features_grid"]->CurrentMode = "edit";
					$GLOBALS["ad_features_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("ad_pictures", $DetailTblVar)) {
				...
				if ($GLOBALS["ad_pictures_grid"]->DetailEdit) {
					$GLOBALS["ad_pictures_grid"]->CurrentMode = "edit";
					$GLOBALS["ad_pictures_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($car_ads_edit)) $car_ads_edit = new ccar_ads_edit();

// Page init
$car_ads_edit->Page_Init();

// Page main
$car_ads_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$car_ads_edit->Page_Render();
?>
...
var CurrentForm = fcar_adsedit = new ew_Form("fcar_adsedit", "edit");

// Validate form
fcar_adsedit.Validate = function() {
...
fcar_adsedit.Form_CustomValidate = 
...
fcar_adsedit.ValidateRequired = true;
<?php } else { ?>
fcar_adsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcar_adsedit.Lists["x_user_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","x__email","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_year_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_year","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_registered_in"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_city_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_city_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_city_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_make_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_model_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_version_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_color_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_engine_type_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_transmition"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_transmition"].Options = <?php echo json_encode($car_ads->transmition->Options()) ?>;
fcar_adsedit.Lists["x_assembly"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_assembly"].Options = <?php echo json_encode($car_ads->assembly->Options()) ?>;
fcar_adsedit.Lists["x_allow_whatsapp"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_allow_whatsapp"].Options = <?php echo json_encode($car_ads->allow_whatsapp->Options()) ?>;
fcar_adsedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcar_adsedit.Lists["x_status"].Options = <?php echo json_encode($car_ads->status->Options()) ?>;
...
<?php $car_ads_edit->ShowPageHeader(); ?>
<?php
$car_ads_edit->ShowMessage();
?>
<form name="fcar_adsedit" id="fcar_adsedit" class="<?php echo $car_ads_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($car_ads_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $car_ads_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fcar_adsedit.CreateAutoSuggest({"id":"x_user_id","forceSelect":false});
</script>
</span>
...
<?php if ($car_ads->ad_title->Visible) { // ad_title ?>
	<div id="r_ad_title" class="form-group">
		<label id="elh_car_ads_ad_title" for="x_ad_title" class="col-sm-2 control-label ewLabel"><?php echo $car_ads->ad_title->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $car_ads->ad_title->CellAttributes() ?>>
...
<?php echo $car_ads->ad_title->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="hidden" data-table="car_ads" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($car_ads->ID->CurrentValue) ?>">
<?php if ($car_ads->getCurrentDetailTable() <> "") { ?>
<?php
	$FirstActiveDetailTable = $car_ads_edit->DetailPages->ActivePageIndex();
?>
<div class="ewDetailPages">
<div class="tabbable" id="car_ads_edit_details">
	<ul class="nav<?php echo $car_ads_edit->DetailPages->NavStyle() ?>">
<?php
	if (in_array("ad_features", explode(",", $car_ads->getCurrentDetailTable())) && $ad_features->DetailEdit) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "ad_features") {
			$FirstActiveDetailTable = "ad_features";
		}
?>
		<li<?php echo $car_ads_edit->DetailPages->TabStyle("ad_features") ?>><a href="#tab_ad_features" data-toggle="tab"><?php echo $Language->TablePhrase("ad_features", "TblCaption") ?></a></li>
<?php
	}
?>
<?php
	if (in_array("ad_pictures", explode(",", $car_ads->getCurrentDetailTable())) && $ad_pictures->DetailEdit) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "ad_pictures") {
			$FirstActiveDetailTable = "ad_pictures";
		}
?>
		<li<?php echo $car_ads_edit->DetailPages->TabStyle("ad_pictures") ?>><a href="#tab_ad_pictures" data-toggle="tab"><?php echo $Language->TablePhrase("ad_pictures", "TblCaption") ?></a></li>
<?php
	}
?>
	</ul>
...
?>
		<div class="tab-pane<?php echo $car_ads_edit->DetailPages->PageStyle("ad_features") ?>" id="tab_ad_features">
...
?>
		<div class="tab-pane<?php echo $car_ads_edit->DetailPages->PageStyle("ad_pictures") ?>" id="tab_ad_pictures">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $car_ads_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
