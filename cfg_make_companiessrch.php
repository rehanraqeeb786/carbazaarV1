...
$cfg_make_companies_search = NULL; // Initialize page object first

class ccfg_make_companies_search extends ccfg_make_companies {
	...
	// Page object name
	var $PageObjName = 'cfg_make_companies_search';
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "cfg_make_companieslist.php" . "?" . $sSrchStr;
						if ($this->IsModal) {
							$row = array();
							$row["url"] = $sSrchStr;
...
// Create page object
if (!isset($cfg_make_companies_search)) $cfg_make_companies_search = new ccfg_make_companies_search();

// Page init
$cfg_make_companies_search->Page_Init();

// Page main
$cfg_make_companies_search->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_make_companies_search->Page_Render();
?>
...
<?php if ($cfg_make_companies_search->IsModal) { ?>
var CurrentAdvancedSearchForm = fcfg_make_companiessearch = new ew_Form("fcfg_make_companiessearch", "search");
<?php } else { ?>
var CurrentForm = fcfg_make_companiessearch = new ew_Form("fcfg_make_companiessearch", "search");
<?php } ?>

// Form_CustomValidate event
fcfg_make_companiessearch.Form_CustomValidate = 
...
fcfg_make_companiessearch.ValidateRequired = true;
<?php } else { ?>
fcfg_make_companiessearch.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_make_companiessearch.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_make_companiessearch.Lists["x_status"].Options = <?php echo json_encode($cfg_make_companies->status->Options()) ?>;

// Form object for search
// Validate function for search

fcfg_make_companiessearch.Validate = function(fobj) {
...
<?php } ?>
<?php $cfg_make_companies_search->ShowPageHeader(); ?>
<?php
$cfg_make_companies_search->ShowMessage();
?>
<form name="fcfg_make_companiessearch" id="fcfg_make_companiessearch" class="<?php echo $cfg_make_companies_search->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_make_companies_search->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_make_companies_search->Token ?>">
<?php } ?>
...
<?php if ($cfg_make_companies->title->Visible) { // title ?>
	<div id="r_title" class="form-group">
		<label for="x_title" class="<?php echo $cfg_make_companies_search->SearchLabelClass ?>"><span id="elh_cfg_make_companies_title"><?php echo $cfg_make_companies->title->FldCaption() ?></span>	
		...
		<div class="<?php echo $cfg_make_companies_search->SearchRightColumnClass ?>"><div<?php echo $cfg_make_companies->title->CellAttributes() ?>>
...
<?php if ($cfg_make_companies->details->Visible) { // details ?>
	<div id="r_details" class="form-group">
		<label for="x_details" class="<?php echo $cfg_make_companies_search->SearchLabelClass ?>"><span id="elh_cfg_make_companies_details"><?php echo $cfg_make_companies->details->FldCaption() ?></span>	
		...
		<div class="<?php echo $cfg_make_companies_search->SearchRightColumnClass ?>"><div<?php echo $cfg_make_companies->details->CellAttributes() ?>>
			<span id="el_cfg_make_companies_details">
<input type="text" data-table="cfg_make_companies" data-field="x_details" name="x_details" id="x_details" size="35" placeholder="<?php echo ew_HtmlEncode($cfg_make_companies->details->getPlaceHolder()) ?>" value="<?php echo $cfg_make_companies->details->EditValue ?>"<?php echo $cfg_make_companies->details->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($cfg_make_companies->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label class="<?php echo $cfg_make_companies_search->SearchLabelClass ?>"><span id="elh_cfg_make_companies_status"><?php echo $cfg_make_companies->status->FldCaption() ?></span>	
		...
		<div class="<?php echo $cfg_make_companies_search->SearchRightColumnClass ?>"><div<?php echo $cfg_make_companies->status->CellAttributes() ?>>
...
if (is_array($arwrk)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($cfg_make_companies->status->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " checked" : "";
		if ($selwrk <> "")
			$emptywrk = FALSE;
?>
...
<script type="text/javascript">
fcfg_make_companiessearch.Init();
</script>
<?php
$cfg_make_companies_search->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_make_companies_search->Page_Terminate();
?>
