...
$classified_faqs = NULL;

//
// Table class for classified_faqs
//
class cclassified_faqs extends cTable {
	var $ID;
	var $classified_id;
	var $question_text;
	var $answer_text;
	...
	function __construct() {
		...
		$this->TableVar = 'classified_faqs';
		$this->TableName = 'classified_faqs';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`classified_faqs`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new cField('classified_faqs', 'classified_faqs', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// classified_id
		$this->classified_id = new cField('classified_faqs', 'classified_faqs', 'x_classified_id', 'classified_id', '`classified_id`', '`classified_id`', 3, -1, FALSE, '`classified_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->classified_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['classified_id'] = &$this->classified_id;

		// question_text
		$this->question_text = new cField('classified_faqs', 'classified_faqs', 'x_question_text', 'question_text', '`question_text`', '`question_text`', 201, -1, FALSE, '`question_text`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['question_text'] = &$this->question_text;

		// answer_text
		$this->answer_text = new cField('classified_faqs', 'classified_faqs', 'x_answer_text', 'answer_text', '`answer_text`', '`answer_text`', 201, -1, FALSE, '`answer_text`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['answer_text'] = &$this->answer_text;

		// ETD
		$this->ETD = new cField('classified_faqs', 'classified_faqs', 'x_ETD', 'ETD', '`ETD`', 'DATE_FORMAT(`ETD`, \'%Y/%m/%d\')', 135, 5, FALSE, '`ETD`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`classified_faqs`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "classified_faqslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("classified_faqsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("classified_faqsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "classified_faqsadd.php?" . $this->UrlParm($parm);
		else
			$url = "classified_faqsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("classified_faqsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("classified_faqsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("classified_faqsdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->question_text->EditValue = $this->question_text->CurrentValue;
		...
		$this->answer_text->EditValue = $this->answer_text->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->question_text->Exportable) $Doc->ExportCaption($this->question_text);
					if ($this->answer_text->Exportable) $Doc->ExportCaption($this->answer_text);
				} else {
					if ($this->classified_id->Exportable) $Doc->ExportCaption($this->classified_id);
				}
				$Doc->EndExportRow();
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->question_text->Exportable) $Doc->ExportField($this->question_text);
						if ($this->answer_text->Exportable) $Doc->ExportField($this->answer_text);
					} else {
						if ($this->classified_id->Exportable) $Doc->ExportField($this->classified_id);
					}
					$Doc->EndExportRow();
