...
$bank_accounts_edit = NULL; // Initialize page object first

class cbank_accounts_edit extends cbank_accounts {
	...
	// Page object name
	var $PageObjName = 'bank_accounts_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("bank_accountslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// bank_account_title
			$this->bank_account_title->SetDbValueDef($rsnew, $this->bank_account_title->CurrentValue, "", $this->bank_account_title->ReadOnly);

			// bank_account
			$this->bank_account->SetDbValueDef($rsnew, $this->bank_account->CurrentValue, "", $this->bank_account->ReadOnly);

			// branch_code
			$this->branch_code->SetDbValueDef($rsnew, $this->branch_code->CurrentValue, NULL, $this->branch_code->ReadOnly);
...
// Create page object
if (!isset($bank_accounts_edit)) $bank_accounts_edit = new cbank_accounts_edit();

// Page init
$bank_accounts_edit->Page_Init();

// Page main
$bank_accounts_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$bank_accounts_edit->Page_Render();
?>
...
var CurrentForm = fbank_accountsedit = new ew_Form("fbank_accountsedit", "edit");

// Validate form
fbank_accountsedit.Validate = function() {
...
fbank_accountsedit.Form_CustomValidate = 
...
fbank_accountsedit.ValidateRequired = true;
<?php } else { ?>
fbank_accountsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fbank_accountsedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fbank_accountsedit.Lists["x_status"].Options = <?php echo json_encode($bank_accounts->status->Options()) ?>;
...
<?php $bank_accounts_edit->ShowPageHeader(); ?>
<?php
$bank_accounts_edit->ShowMessage();
?>
<form name="fbank_accountsedit" id="fbank_accountsedit" class="<?php echo $bank_accounts_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($bank_accounts_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $bank_accounts_edit->Token ?>">
<?php } ?>
...
<input type="hidden" data-table="bank_accounts" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($bank_accounts->id->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $bank_accounts_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fbank_accountsedit.Init();
</script>
<?php
$bank_accounts_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$bank_accounts_edit->Page_Terminate();
?>
