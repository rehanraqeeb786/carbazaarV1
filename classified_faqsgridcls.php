...
$classified_faqs_grid = NULL; // Initialize page object first

class cclassified_faqs_grid extends cclassified_faqs {
	...
	// Page object name
	var $PageObjName = 'classified_faqs_grid';

	// Grid form hidden field names
	var $FormName = 'fclassified_faqsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["classified_faqs"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_classified_id") && $objForm->HasValue("o_classified_id") && $this->classified_id->CurrentValue <> $this->classified_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_question_text") && $objForm->HasValue("o_question_text") && $this->question_text->CurrentValue <> $this->question_text->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_answer_text") && $objForm->HasValue("o_answer_text") && $this->answer_text->CurrentValue <> $this->answer_text->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "classified_data") {
			$this->classified_id->Visible = FALSE;
			if ($GLOBALS["classified_data"]->EventCancelled) $this->EventCancelled = TRUE;
		}
