...
$dealers_showroom_add = NULL; // Initialize page object first

class cdealers_showroom_add extends cdealers_showroom {
	...
	// Page object name
	var $PageObjName = 'dealers_showroom_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "dealers_showroomview.php")
	...
	function LoadDefaultValues() {
		...
		$this->showroom_name->CurrentValue = NULL;
		$this->showroom_name->OldValue = $this->showroom_name->CurrentValue;
		$this->showroom_address->CurrentValue = NULL;
		$this->showroom_address->OldValue = $this->showroom_address->CurrentValue;
		$this->city_id->CurrentValue = NULL;
		$this->city_id->OldValue = $this->city_id->CurrentValue;
		$this->owner_name->CurrentValue = NULL;
		$this->owner_name->OldValue = $this->owner_name->CurrentValue;
		$this->contact_1->CurrentValue = NULL;
		$this->contact_1->OldValue = $this->contact_1->CurrentValue;
		$this->contat_2->CurrentValue = NULL;
		$this->contat_2->OldValue = $this->contat_2->CurrentValue;
		$this->showroom_logo_link->CurrentValue = NULL;
		$this->showroom_logo_link->OldValue = $this->showroom_logo_link->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// showroom_name
		$this->showroom_name->SetDbValueDef($rsnew, $this->showroom_name->CurrentValue, "", FALSE);

		// showroom_address
		$this->showroom_address->SetDbValueDef($rsnew, $this->showroom_address->CurrentValue, "", FALSE);

		// city_id
		$this->city_id->SetDbValueDef($rsnew, $this->city_id->CurrentValue, 0, FALSE);

		// owner_name
		$this->owner_name->SetDbValueDef($rsnew, $this->owner_name->CurrentValue, "", FALSE);

		// contact_1
		$this->contact_1->SetDbValueDef($rsnew, $this->contact_1->CurrentValue, "", FALSE);

		// contat_2
		$this->contat_2->SetDbValueDef($rsnew, $this->contat_2->CurrentValue, NULL, FALSE);

		// showroom_logo_link
		$this->showroom_logo_link->SetDbValueDef($rsnew, $this->showroom_logo_link->CurrentValue, NULL, FALSE);

		// status
		$this->status->SetDbValueDef($rsnew, ((strval($this->status->CurrentValue) == "1") ? "1" : "0"), 0, strval($this->status->CurrentValue) == "");

		// created_at
		$this->created_at->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->created_at->CurrentValue, 5), NULL, FALSE);

		// updated_at
		$this->updated_at->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->updated_at->CurrentValue, 5), NULL, FALSE);
...
// Create page object
if (!isset($dealers_showroom_add)) $dealers_showroom_add = new cdealers_showroom_add();

// Page init
$dealers_showroom_add->Page_Init();

// Page main
$dealers_showroom_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$dealers_showroom_add->Page_Render();
?>
...
var CurrentForm = fdealers_showroomadd = new ew_Form("fdealers_showroomadd", "add");

// Validate form
fdealers_showroomadd.Validate = function() {
...
fdealers_showroomadd.Form_CustomValidate = 
...
fdealers_showroomadd.ValidateRequired = true;
<?php } else { ?>
fdealers_showroomadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdealers_showroomadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdealers_showroomadd.Lists["x_status"].Options = <?php echo json_encode($dealers_showroom->status->Options()) ?>;
...
<?php $dealers_showroom_add->ShowPageHeader(); ?>
<?php
$dealers_showroom_add->ShowMessage();
?>
<form name="fdealers_showroomadd" id="fdealers_showroomadd" class="<?php echo $dealers_showroom_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($dealers_showroom_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $dealers_showroom_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $dealers_showroom_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fdealers_showroomadd.Init();
</script>
<?php
$dealers_showroom_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$dealers_showroom_add->Page_Terminate();
?>
