...
$classified_data_add = NULL; // Initialize page object first

class cclassified_data_add extends cclassified_data {
	...
	// Page object name
	var $PageObjName = 'classified_data_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "classified_dataview.php")
	...
	function LoadDefaultValues() {
		...
		$this->car_type->CurrentValue = NULL;
		$this->car_type->OldValue = $this->car_type->CurrentValue;
		$this->car_make_company_id->CurrentValue = NULL;
		$this->car_make_company_id->OldValue = $this->car_make_company_id->CurrentValue;
		$this->car_model_id->CurrentValue = NULL;
		$this->car_model_id->OldValue = $this->car_model_id->CurrentValue;
		$this->price_range->CurrentValue = NULL;
		$this->price_range->OldValue = $this->price_range->CurrentValue;
		$this->milage_km_liter->CurrentValue = NULL;
		$this->milage_km_liter->OldValue = $this->milage_km_liter->CurrentValue;
		$this->transmition->CurrentValue = NULL;
		$this->transmition->OldValue = $this->transmition->CurrentValue;
		$this->fuel_type->CurrentValue = "Petrol";
		$this->engine_capicity->CurrentValue = NULL;
		$this->engine_capicity->OldValue = $this->engine_capicity->CurrentValue;
		$this->detail_text->CurrentValue = NULL;
		$this->detail_text->OldValue = $this->detail_text->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("classified_colors", $DetailTblVar) && $GLOBALS["classified_colors"]->DetailAdd) {
		...
		if (in_array("classified_attributes", $DetailTblVar) && $GLOBALS["classified_attributes"]->DetailAdd) {
		...
		if (in_array("classified_faqs", $DetailTblVar) && $GLOBALS["classified_faqs"]->DetailAdd) {
		...
		if (in_array("classified_pictures", $DetailTblVar) && $GLOBALS["classified_pictures"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// car_type
		$this->car_type->SetDbValueDef($rsnew, $this->car_type->CurrentValue, NULL, FALSE);

		// car_make_company_id
		$this->car_make_company_id->SetDbValueDef($rsnew, $this->car_make_company_id->CurrentValue, 0, FALSE);

		// car_model_id
		$this->car_model_id->SetDbValueDef($rsnew, $this->car_model_id->CurrentValue, 0, FALSE);

		// price_range
		$this->price_range->SetDbValueDef($rsnew, $this->price_range->CurrentValue, "", FALSE);

		// milage_km_liter
		$this->milage_km_liter->SetDbValueDef($rsnew, $this->milage_km_liter->CurrentValue, "", FALSE);

		// transmition
		$this->transmition->SetDbValueDef($rsnew, $this->transmition->CurrentValue, NULL, FALSE);

		// fuel_type
		$this->fuel_type->SetDbValueDef($rsnew, $this->fuel_type->CurrentValue, "", strval($this->fuel_type->CurrentValue) == "");

		// engine_capicity
		$this->engine_capicity->SetDbValueDef($rsnew, $this->engine_capicity->CurrentValue, 0, FALSE);

		// detail_text
		$this->detail_text->SetDbValueDef($rsnew, $this->detail_text->CurrentValue, "", FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("classified_colors", $DetailTblVar) && $GLOBALS["classified_colors"]->DetailAdd) {
				$GLOBALS["classified_colors"]->classfied_id->setSessionValue($this->ID->CurrentValue); // Set master key
				if (!isset($GLOBALS["classified_colors_grid"])) $GLOBALS["classified_colors_grid"] = new cclassified_colors_grid(); // Get detail page object
				$AddRow = $GLOBALS["classified_colors_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["classified_colors"]->classfied_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("classified_attributes", $DetailTblVar) && $GLOBALS["classified_attributes"]->DetailAdd) {
				$GLOBALS["classified_attributes"]->classified_id->setSessionValue($this->ID->CurrentValue); // Set master key
				if (!isset($GLOBALS["classified_attributes_grid"])) $GLOBALS["classified_attributes_grid"] = new cclassified_attributes_grid(); // Get detail page object
				$AddRow = $GLOBALS["classified_attributes_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["classified_attributes"]->classified_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("classified_faqs", $DetailTblVar) && $GLOBALS["classified_faqs"]->DetailAdd) {
				$GLOBALS["classified_faqs"]->classified_id->setSessionValue($this->ID->CurrentValue); // Set master key
				if (!isset($GLOBALS["classified_faqs_grid"])) $GLOBALS["classified_faqs_grid"] = new cclassified_faqs_grid(); // Get detail page object
				$AddRow = $GLOBALS["classified_faqs_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["classified_faqs"]->classified_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("classified_pictures", $DetailTblVar) && $GLOBALS["classified_pictures"]->DetailAdd) {
				$GLOBALS["classified_pictures"]->classified_id->setSessionValue($this->ID->CurrentValue); // Set master key
				if (!isset($GLOBALS["classified_pictures_grid"])) $GLOBALS["classified_pictures_grid"] = new cclassified_pictures_grid(); // Get detail page object
				$AddRow = $GLOBALS["classified_pictures_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["classified_pictures"]->classified_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("classified_colors", $DetailTblVar)) {
				...
				if ($GLOBALS["classified_colors_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["classified_colors_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["classified_colors_grid"]->CurrentMode = "add";
					$GLOBALS["classified_colors_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("classified_attributes", $DetailTblVar)) {
				...
				if ($GLOBALS["classified_attributes_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["classified_attributes_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["classified_attributes_grid"]->CurrentMode = "add";
					$GLOBALS["classified_attributes_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("classified_faqs", $DetailTblVar)) {
				...
				if ($GLOBALS["classified_faqs_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["classified_faqs_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["classified_faqs_grid"]->CurrentMode = "add";
					$GLOBALS["classified_faqs_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("classified_pictures", $DetailTblVar)) {
				...
				if ($GLOBALS["classified_pictures_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["classified_pictures_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["classified_pictures_grid"]->CurrentMode = "add";
					$GLOBALS["classified_pictures_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($classified_data_add)) $classified_data_add = new cclassified_data_add();

// Page init
$classified_data_add->Page_Init();

// Page main
$classified_data_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$classified_data_add->Page_Render();
?>
...
var CurrentForm = fclassified_dataadd = new ew_Form("fclassified_dataadd", "add");

// Validate form
fclassified_dataadd.Validate = function() {
...
fclassified_dataadd.Form_CustomValidate = 
...
fclassified_dataadd.ValidateRequired = true;
<?php } else { ?>
fclassified_dataadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fclassified_dataadd.Lists["x_car_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fclassified_dataadd.Lists["x_car_type"].Options = <?php echo json_encode($classified_data->car_type->Options()) ?>;
fclassified_dataadd.Lists["x_car_make_company_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":["x_car_model_id"],"FilterFields":[],"Options":[],"Template":""};
fclassified_dataadd.Lists["x_car_model_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":["x_car_make_company_id"],"ChildFields":[],"FilterFields":["x_make_company_id"],"Options":[],"Template":""};
fclassified_dataadd.Lists["x_transmition"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fclassified_dataadd.Lists["x_transmition"].Options = <?php echo json_encode($classified_data->transmition->Options()) ?>;
fclassified_dataadd.Lists["x_fuel_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fclassified_dataadd.Lists["x_fuel_type"].Options = <?php echo json_encode($classified_data->fuel_type->Options()) ?>;
fclassified_dataadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fclassified_dataadd.Lists["x_status"].Options = <?php echo json_encode($classified_data->status->Options()) ?>;
...
<?php $classified_data_add->ShowPageHeader(); ?>
<?php
$classified_data_add->ShowMessage();
?>
<form name="fclassified_dataadd" id="fclassified_dataadd" class="<?php echo $classified_data_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($classified_data_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $classified_data_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fclassified_dataadd", "x_detail_text", 0, 0, <?php echo ($classified_data->detail_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	$FirstActiveDetailTable = $classified_data_add->DetailPages->ActivePageIndex();
?>
<div class="ewDetailPages">
<div class="tabbable" id="classified_data_add_details">
	<ul class="nav<?php echo $classified_data_add->DetailPages->NavStyle() ?>">
<?php
	if (in_array("classified_colors", explode(",", $classified_data->getCurrentDetailTable())) && $classified_colors->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "classified_colors") {
			$FirstActiveDetailTable = "classified_colors";
		}
?>
		<li<?php echo $classified_data_add->DetailPages->TabStyle("classified_colors") ?>><a href="#tab_classified_colors" data-toggle="tab"><?php echo $Language->TablePhrase("classified_colors", "TblCaption") ?></a></li>
<?php
	}
?>
<?php
	if (in_array("classified_attributes", explode(",", $classified_data->getCurrentDetailTable())) && $classified_attributes->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "classified_attributes") {
			$FirstActiveDetailTable = "classified_attributes";
		}
?>
		<li<?php echo $classified_data_add->DetailPages->TabStyle("classified_attributes") ?>><a href="#tab_classified_attributes" data-toggle="tab"><?php echo $Language->TablePhrase("classified_attributes", "TblCaption") ?></a></li>
<?php
	}
?>
<?php
	if (in_array("classified_faqs", explode(",", $classified_data->getCurrentDetailTable())) && $classified_faqs->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "classified_faqs") {
			$FirstActiveDetailTable = "classified_faqs";
		}
?>
		<li<?php echo $classified_data_add->DetailPages->TabStyle("classified_faqs") ?>><a href="#tab_classified_faqs" data-toggle="tab"><?php echo $Language->TablePhrase("classified_faqs", "TblCaption") ?></a></li>
<?php
	}
?>
<?php
	if (in_array("classified_pictures", explode(",", $classified_data->getCurrentDetailTable())) && $classified_pictures->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "classified_pictures") {
			$FirstActiveDetailTable = "classified_pictures";
		}
?>
		<li<?php echo $classified_data_add->DetailPages->TabStyle("classified_pictures") ?>><a href="#tab_classified_pictures" data-toggle="tab"><?php echo $Language->TablePhrase("classified_pictures", "TblCaption") ?></a></li>
<?php
	}
?>
	</ul>
...
?>
		<div class="tab-pane<?php echo $classified_data_add->DetailPages->PageStyle("classified_colors") ?>" id="tab_classified_colors">
...
?>
		<div class="tab-pane<?php echo $classified_data_add->DetailPages->PageStyle("classified_attributes") ?>" id="tab_classified_attributes">
...
?>
		<div class="tab-pane<?php echo $classified_data_add->DetailPages->PageStyle("classified_faqs") ?>" id="tab_classified_faqs">
...
?>
		<div class="tab-pane<?php echo $classified_data_add->DetailPages->PageStyle("classified_pictures") ?>" id="tab_classified_pictures">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $classified_data_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fclassified_dataadd.Init();
</script>
<?php
$classified_data_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$classified_data_add->Page_Terminate();
?>
