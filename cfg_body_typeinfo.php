...
//
// Table class for cfg_body_type
//
class ccfg_body_type extends cTable {
	var $ID;
	var $title;
	var $details;
	var $icon_url;
	...
	function __construct() {
		...
		$this->TableVar = 'cfg_body_type';
		$this->TableName = 'cfg_body_type';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cfg_body_type`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new cField('cfg_body_type', 'cfg_body_type', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// title
		$this->title = new cField('cfg_body_type', 'cfg_body_type', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// details
		$this->details = new cField('cfg_body_type', 'cfg_body_type', 'x_details', 'details', '`details`', '`details`', 201, -1, FALSE, '`details`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['details'] = &$this->details;

		// icon_url
		$this->icon_url = new cField('cfg_body_type', 'cfg_body_type', 'x_icon_url', 'icon_url', '`icon_url`', '`icon_url`', 200, -1, TRUE, '`icon_url`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['icon_url'] = &$this->icon_url;

		// status
		$this->status = new cField('cfg_body_type', 'cfg_body_type', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->status->OptionCount = 2;
		$this->fields['status'] = &$this->status;

		// created_at
		$this->created_at = new cField('cfg_body_type', 'cfg_body_type', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('cfg_body_type', 'cfg_body_type', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cfg_body_type`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cfg_body_typeview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cfg_body_typeview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cfg_body_typeadd.php?" . $this->UrlParm($parm);
		else
			$url = "cfg_body_typeadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("cfg_body_typeedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("cfg_body_typeadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cfg_body_typedelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->icon_url->Exportable) $Doc->ExportCaption($this->icon_url);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->icon_url->Exportable) $Doc->ExportField($this->icon_url);
