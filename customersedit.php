...
class ccustomers_edit extends ccustomers {
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->profile_pic);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// email_id
			$this->email_id->SetDbValueDef($rsnew, $this->email_id->CurrentValue, "", $this->email_id->ReadOnly);
			...
			// profile_pic
			if ($this->profile_pic->Visible && !$this->profile_pic->ReadOnly && !$this->profile_pic->Upload->KeepFile) {
				$this->profile_pic->Upload->DbValue = $rsold['profile_pic']; // Get original value
			...
			// dob
			$this->dob->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->dob->CurrentValue, 7), NULL, $this->dob->ReadOnly);

			// city_id
			$this->city_id->SetDbValueDef($rsnew, $this->city_id->CurrentValue, NULL, $this->city_id->ReadOnly);

			// customer_type
			$this->customer_type->SetDbValueDef($rsnew, $this->customer_type->CurrentValue, "", $this->customer_type->ReadOnly);

			// mobile_num
			$this->mobile_num->SetDbValueDef($rsnew, $this->mobile_num->CurrentValue, NULL, $this->mobile_num->ReadOnly);
...
fcustomersedit.Lists["x_city_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_city_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcustomersedit.Lists["x_customer_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcustomersedit.Lists["x_customer_type"].Options = <?php echo json_encode($customers->customer_type->Options()) ?>;
fcustomersedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcustomersedit.Lists["x_status"].Options = <?php echo json_encode($customers->status->Options()) ?>;
...
<?php if (@$_POST["fa_x_profile_pic"] == "0") { ?>
<input type="hidden" name="fa_x_profile_pic" id= "fa_x_profile_pic" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_profile_pic" id= "fa_x_profile_pic" value="1">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fcustomersedit", "x_dob", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<input type="hidden" data-table="customers" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($customers->ID->CurrentValue) ?>">
