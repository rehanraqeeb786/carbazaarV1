...
//
// Table class for ad_pictures
//
class cad_pictures extends cTable {
	var $ID;
	var $ad_id;
	var $picture_link;
	var $created_at;
	...
	function __construct() {
		...
		$this->TableVar = 'ad_pictures';
		$this->TableName = 'ad_pictures';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`ad_pictures`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new cField('ad_pictures', 'ad_pictures', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// ad_id
		$this->ad_id = new cField('ad_pictures', 'ad_pictures', 'x_ad_id', 'ad_id', '`ad_id`', '`ad_id`', 3, -1, FALSE, '`ad_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ad_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ad_id'] = &$this->ad_id;

		// picture_link
		$this->picture_link = new cField('ad_pictures', 'ad_pictures', 'x_picture_link', 'picture_link', '`picture_link`', '`picture_link`', 200, -1, TRUE, '`picture_link`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->fields['picture_link'] = &$this->picture_link;

		// created_at
		$this->created_at = new cField('ad_pictures', 'ad_pictures', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('ad_pictures', 'ad_pictures', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "car_ads") {
			if ($this->ad_id->getSessionValue() <> "")
				$sMasterFilter .= "`ID`=" . ew_QuotedValue($this->ad_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "car_ads") {
			if ($this->ad_id->getSessionValue() <> "")
				$sDetailFilter .= "`ad_id`=" . ew_QuotedValue($this->ad_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_car_ads() {
		return "`ID`=@ID@";
	}

	// Detail filter
	function SqlDetailFilter_car_ads() {
		return "`ad_id`=@ad_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`ad_pictures`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("ad_picturesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("ad_picturesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "ad_picturesadd.php?" . $this->UrlParm($parm);
		else
			$url = "ad_picturesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("ad_picturesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("ad_picturesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("ad_picturesdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "car_ads" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_ID=" . urlencode($this->ad_id->CurrentValue);
		}
		return $url;
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->picture_link->Exportable) $Doc->ExportCaption($this->picture_link);
				} else {
					if ($this->ad_id->Exportable) $Doc->ExportCaption($this->ad_id);
					if ($this->picture_link->Exportable) $Doc->ExportCaption($this->picture_link);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->picture_link->Exportable) $Doc->ExportField($this->picture_link);
					} else {
						if ($this->ad_id->Exportable) $Doc->ExportField($this->ad_id);
						if ($this->picture_link->Exportable) $Doc->ExportField($this->picture_link);
					}
