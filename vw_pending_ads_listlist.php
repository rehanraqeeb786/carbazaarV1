...
$vw_pending_ads_list_list = NULL; // Initialize page object first

class cvw_pending_ads_list_list extends cvw_pending_ads_list {
	...
	// Page object name
	var $PageObjName = 'vw_pending_ads_list_list';

	// Grid form hidden field names
	var $FormName = 'fvw_pending_ads_listlist';
	...
	function __construct() {
		...
		$this->AddUrl = "vw_pending_ads_listadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "vw_pending_ads_listdelete.php";
		$this->MultiUpdateUrl = "vw_pending_ads_listupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fvw_pending_ads_listlistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fvw_pending_ads_listlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fvw_pending_ads_listlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fvw_pending_ads_listlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"vw_pending_ads_listsrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;
	...
	function Page_Load() {
		...
		//echo "Page Load";
		$this->CustomActions["approve_ad"] = "Approve";
	}
	...
	function Row_CustomAction($action, $row) {
		...
		if($action == 'approve_ad')
		{
			...
			ew_execute("UPDATE `car_ads` SET `status` = 'Approved' WHERE `car_ads`.`ID` = ".$row['ID']);
		}
		return TRUE;
	}
...
// Create page object
if (!isset($vw_pending_ads_list_list)) $vw_pending_ads_list_list = new cvw_pending_ads_list_list();

// Page init
$vw_pending_ads_list_list->Page_Init();

// Page main
$vw_pending_ads_list_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$vw_pending_ads_list_list->Page_Render();
?>
...
var CurrentForm = fvw_pending_ads_listlist = new ew_Form("fvw_pending_ads_listlist", "list");
fvw_pending_ads_listlist.FormKeyCountName = '<?php echo $vw_pending_ads_list_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvw_pending_ads_listlist.Form_CustomValidate = 
...
fvw_pending_ads_listlist.ValidateRequired = true;
<?php } else { ?>
fvw_pending_ads_listlist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvw_pending_ads_listlist.Lists["x_user_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","x__email","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_pending_ads_listlist.Lists["x_year_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_year","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_pending_ads_listlist.Lists["x_registered_in"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_city_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_pending_ads_listlist.Lists["x_city_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_city_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_pending_ads_listlist.Lists["x_make_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fvw_pending_ads_listlist.Lists["x_model_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};

// Form object for search
var CurrentSearchForm = fvw_pending_ads_listlistsrch = new ew_Form("fvw_pending_ads_listlistsrch");
</script>
<script type="text/javascript">
...
<?php if ($vw_pending_ads_list_list->TotalRecs > 0 && $vw_pending_ads_list_list->ExportOptions->Visible()) { ?>
<?php $vw_pending_ads_list_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_pending_ads_list_list->SearchOptions->Visible()) { ?>
<?php $vw_pending_ads_list_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($vw_pending_ads_list_list->FilterOptions->Visible()) { ?>
<?php $vw_pending_ads_list_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php
	$bSelectLimit = $vw_pending_ads_list_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($vw_pending_ads_list_list->TotalRecs <= 0)
			$vw_pending_ads_list_list->TotalRecs = $vw_pending_ads_list->SelectRecordCount();
	} else {
		if (!$vw_pending_ads_list_list->Recordset && ($vw_pending_ads_list_list->Recordset = $vw_pending_ads_list_list->LoadRecordset()))
			$vw_pending_ads_list_list->TotalRecs = $vw_pending_ads_list_list->Recordset->RecordCount();
	}
	$vw_pending_ads_list_list->StartRec = 1;
	if ($vw_pending_ads_list_list->DisplayRecs <= 0 || ($vw_pending_ads_list->Export <> "" && $vw_pending_ads_list->ExportAll)) // Display all records
		$vw_pending_ads_list_list->DisplayRecs = $vw_pending_ads_list_list->TotalRecs;
	if (!($vw_pending_ads_list->Export <> "" && $vw_pending_ads_list->ExportAll))
		$vw_pending_ads_list_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$vw_pending_ads_list_list->Recordset = $vw_pending_ads_list_list->LoadRecordset($vw_pending_ads_list_list->StartRec-1, $vw_pending_ads_list_list->DisplayRecs);

	// Set no record found message
	if ($vw_pending_ads_list->CurrentAction == "" && $vw_pending_ads_list_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$vw_pending_ads_list_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($vw_pending_ads_list_list->SearchWhere == "0=101")
			$vw_pending_ads_list_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$vw_pending_ads_list_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$vw_pending_ads_list_list->RenderOtherOptions();
?>
<?php $vw_pending_ads_list_list->ShowPageHeader(); ?>
<?php
$vw_pending_ads_list_list->ShowMessage();
?>
<?php if ($vw_pending_ads_list_list->TotalRecs > 0 || $vw_pending_ads_list->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid">
<form name="fvw_pending_ads_listlist" id="fvw_pending_ads_listlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($vw_pending_ads_list_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $vw_pending_ads_list_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="vw_pending_ads_list">
<div id="gmp_vw_pending_ads_list" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($vw_pending_ads_list_list->TotalRecs > 0) { ?>
<table id="tbl_vw_pending_ads_listlist" class="table ewTable">
<?php echo $vw_pending_ads_list->TableCustomInnerHtml ?>
<thead><!-- Table header -->
	<tr class="ewTableHeader">
<?php

// Header row
$vw_pending_ads_list_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$vw_pending_ads_list_list->RenderListOptions();

// Render list options (header, left)
$vw_pending_ads_list_list->ListOptions->Render("header", "left");
?>
<?php if ($vw_pending_ads_list->user_id->Visible) { // user_id ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->user_id) == "") { ?>
		<th data-name="user_id"><div id="elh_vw_pending_ads_list_user_id" class="vw_pending_ads_list_user_id"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->user_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="user_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->user_id) ?>',1);"><div id="elh_vw_pending_ads_list_user_id" class="vw_pending_ads_list_user_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->user_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->user_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->user_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->ad_title->Visible) { // ad_title ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->ad_title) == "") { ?>
		<th data-name="ad_title"><div id="elh_vw_pending_ads_list_ad_title" class="vw_pending_ads_list_ad_title"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->ad_title->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="ad_title"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->ad_title) ?>',1);"><div id="elh_vw_pending_ads_list_ad_title" class="vw_pending_ads_list_ad_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->ad_title->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->ad_title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->ad_title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->year_id->Visible) { // year_id ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->year_id) == "") { ?>
		<th data-name="year_id"><div id="elh_vw_pending_ads_list_year_id" class="vw_pending_ads_list_year_id"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->year_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="year_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->year_id) ?>',1);"><div id="elh_vw_pending_ads_list_year_id" class="vw_pending_ads_list_year_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->year_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->year_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->year_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->registered_in->Visible) { // registered_in ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->registered_in) == "") { ?>
		<th data-name="registered_in"><div id="elh_vw_pending_ads_list_registered_in" class="vw_pending_ads_list_registered_in"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->registered_in->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="registered_in"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->registered_in) ?>',1);"><div id="elh_vw_pending_ads_list_registered_in" class="vw_pending_ads_list_registered_in">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->registered_in->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->registered_in->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->registered_in->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->city_id->Visible) { // city_id ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->city_id) == "") { ?>
		<th data-name="city_id"><div id="elh_vw_pending_ads_list_city_id" class="vw_pending_ads_list_city_id"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->city_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="city_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->city_id) ?>',1);"><div id="elh_vw_pending_ads_list_city_id" class="vw_pending_ads_list_city_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->city_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->city_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->city_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->make_id->Visible) { // make_id ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->make_id) == "") { ?>
		<th data-name="make_id"><div id="elh_vw_pending_ads_list_make_id" class="vw_pending_ads_list_make_id"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->make_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="make_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->make_id) ?>',1);"><div id="elh_vw_pending_ads_list_make_id" class="vw_pending_ads_list_make_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->make_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->make_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->make_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->model_id->Visible) { // model_id ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->model_id) == "") { ?>
		<th data-name="model_id"><div id="elh_vw_pending_ads_list_model_id" class="vw_pending_ads_list_model_id"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->model_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="model_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->model_id) ?>',1);"><div id="elh_vw_pending_ads_list_model_id" class="vw_pending_ads_list_model_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->model_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->model_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->model_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->demand_price->Visible) { // demand_price ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->demand_price) == "") { ?>
		<th data-name="demand_price"><div id="elh_vw_pending_ads_list_demand_price" class="vw_pending_ads_list_demand_price"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->demand_price->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="demand_price"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->demand_price) ?>',1);"><div id="elh_vw_pending_ads_list_demand_price" class="vw_pending_ads_list_demand_price">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->demand_price->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->demand_price->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->demand_price->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($vw_pending_ads_list->name->Visible) { // name ?>
	<?php if ($vw_pending_ads_list->SortUrl($vw_pending_ads_list->name) == "") { ?>
		<th data-name="name"><div id="elh_vw_pending_ads_list_name" class="vw_pending_ads_list_name"><div class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $vw_pending_ads_list->SortUrl($vw_pending_ads_list->name) ?>',1);"><div id="elh_vw_pending_ads_list_name" class="vw_pending_ads_list_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $vw_pending_ads_list->name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($vw_pending_ads_list->name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($vw_pending_ads_list->name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$vw_pending_ads_list_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($vw_pending_ads_list->ExportAll && $vw_pending_ads_list->Export <> "") {
	$vw_pending_ads_list_list->StopRec = $vw_pending_ads_list_list->TotalRecs;
} else {

	// Set the last record to display
	if ($vw_pending_ads_list_list->TotalRecs > $vw_pending_ads_list_list->StartRec + $vw_pending_ads_list_list->DisplayRecs - 1)
		$vw_pending_ads_list_list->StopRec = $vw_pending_ads_list_list->StartRec + $vw_pending_ads_list_list->DisplayRecs - 1;
	else
		$vw_pending_ads_list_list->StopRec = $vw_pending_ads_list_list->TotalRecs;
}
$vw_pending_ads_list_list->RecCnt = $vw_pending_ads_list_list->StartRec - 1;
if ($vw_pending_ads_list_list->Recordset && !$vw_pending_ads_list_list->Recordset->EOF) {
	$vw_pending_ads_list_list->Recordset->MoveFirst();
	$bSelectLimit = $vw_pending_ads_list_list->UseSelectLimit;
	if (!$bSelectLimit && $vw_pending_ads_list_list->StartRec > 1)
		$vw_pending_ads_list_list->Recordset->Move($vw_pending_ads_list_list->StartRec - 1);
} elseif (!$vw_pending_ads_list->AllowAddDeleteRow && $vw_pending_ads_list_list->StopRec == 0) {
	$vw_pending_ads_list_list->StopRec = $vw_pending_ads_list->GridAddRowCount;
}

// Initialize aggregate
$vw_pending_ads_list->RowType = EW_ROWTYPE_AGGREGATEINIT;
$vw_pending_ads_list->ResetAttrs();
$vw_pending_ads_list_list->RenderRow();
while ($vw_pending_ads_list_list->RecCnt < $vw_pending_ads_list_list->StopRec) {
	$vw_pending_ads_list_list->RecCnt++;
	if (intval($vw_pending_ads_list_list->RecCnt) >= intval($vw_pending_ads_list_list->StartRec)) {
		$vw_pending_ads_list_list->RowCnt++;

		// Set up key count
		$vw_pending_ads_list_list->KeyCount = $vw_pending_ads_list_list->RowIndex;

		// Init row class and style
		$vw_pending_ads_list->ResetAttrs();
		$vw_pending_ads_list->CssClass = "";
		if ($vw_pending_ads_list->CurrentAction == "gridadd") {
		} else {
			$vw_pending_ads_list_list->LoadRowValues($vw_pending_ads_list_list->Recordset); // Load row values
		}
		$vw_pending_ads_list->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$vw_pending_ads_list->RowAttrs = array_merge($vw_pending_ads_list->RowAttrs, array('data-rowindex'=>$vw_pending_ads_list_list->RowCnt, 'id'=>'r' . $vw_pending_ads_list_list->RowCnt . '_vw_pending_ads_list', 'data-rowtype'=>$vw_pending_ads_list->RowType));
		...
		$vw_pending_ads_list_list->RenderListOptions();
?>
	<tr<?php echo $vw_pending_ads_list->RowAttributes() ?>>
<?php

// Render list options (body, left)
$vw_pending_ads_list_list->ListOptions->Render("body", "left", $vw_pending_ads_list_list->RowCnt);
?>
	<?php if ($vw_pending_ads_list->user_id->Visible) { // user_id ?>
		<td data-name="user_id"<?php echo $vw_pending_ads_list->user_id->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_user_id" class="vw_pending_ads_list_user_id">
<span<?php echo $vw_pending_ads_list->user_id->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->user_id->ListViewValue() ?></span>
</span>
<a id="<?php echo $vw_pending_ads_list_list->PageObjName . "_row_" . $vw_pending_ads_list_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->ad_title->Visible) { // ad_title ?>
		<td data-name="ad_title"<?php echo $vw_pending_ads_list->ad_title->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_ad_title" class="vw_pending_ads_list_ad_title">
<span<?php echo $vw_pending_ads_list->ad_title->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->ad_title->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->year_id->Visible) { // year_id ?>
		<td data-name="year_id"<?php echo $vw_pending_ads_list->year_id->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_year_id" class="vw_pending_ads_list_year_id">
<span<?php echo $vw_pending_ads_list->year_id->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->year_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->registered_in->Visible) { // registered_in ?>
		<td data-name="registered_in"<?php echo $vw_pending_ads_list->registered_in->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_registered_in" class="vw_pending_ads_list_registered_in">
<span<?php echo $vw_pending_ads_list->registered_in->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->registered_in->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->city_id->Visible) { // city_id ?>
		<td data-name="city_id"<?php echo $vw_pending_ads_list->city_id->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_city_id" class="vw_pending_ads_list_city_id">
<span<?php echo $vw_pending_ads_list->city_id->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->city_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->make_id->Visible) { // make_id ?>
		<td data-name="make_id"<?php echo $vw_pending_ads_list->make_id->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_make_id" class="vw_pending_ads_list_make_id">
<span<?php echo $vw_pending_ads_list->make_id->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->make_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->model_id->Visible) { // model_id ?>
		<td data-name="model_id"<?php echo $vw_pending_ads_list->model_id->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_model_id" class="vw_pending_ads_list_model_id">
<span<?php echo $vw_pending_ads_list->model_id->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->model_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->demand_price->Visible) { // demand_price ?>
		<td data-name="demand_price"<?php echo $vw_pending_ads_list->demand_price->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_demand_price" class="vw_pending_ads_list_demand_price">
<span<?php echo $vw_pending_ads_list->demand_price->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->demand_price->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($vw_pending_ads_list->name->Visible) { // name ?>
		<td data-name="name"<?php echo $vw_pending_ads_list->name->CellAttributes() ?>>
<span id="el<?php echo $vw_pending_ads_list_list->RowCnt ?>_vw_pending_ads_list_name" class="vw_pending_ads_list_name">
<span<?php echo $vw_pending_ads_list->name->ViewAttributes() ?>>
<?php echo $vw_pending_ads_list->name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$vw_pending_ads_list_list->ListOptions->Render("body", "right", $vw_pending_ads_list_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($vw_pending_ads_list->CurrentAction <> "gridadd")
		$vw_pending_ads_list_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($vw_pending_ads_list->CurrentAction == "") { ?>
...
// Close recordset
if ($vw_pending_ads_list_list->Recordset)
	$vw_pending_ads_list_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($vw_pending_ads_list->CurrentAction <> "gridadd" && $vw_pending_ads_list->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($vw_pending_ads_list_list->Pager)) $vw_pending_ads_list_list->Pager = new cPrevNextPager($vw_pending_ads_list_list->StartRec, $vw_pending_ads_list_list->DisplayRecs, $vw_pending_ads_list_list->TotalRecs) ?>
<?php if ($vw_pending_ads_list_list->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($vw_pending_ads_list_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $vw_pending_ads_list_list->PageUrl() ?>start=<?php echo $vw_pending_ads_list_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($vw_pending_ads_list_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $vw_pending_ads_list_list->PageUrl() ?>start=<?php echo $vw_pending_ads_list_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $vw_pending_ads_list_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($vw_pending_ads_list_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $vw_pending_ads_list_list->PageUrl() ?>start=<?php echo $vw_pending_ads_list_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($vw_pending_ads_list_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $vw_pending_ads_list_list->PageUrl() ?>start=<?php echo $vw_pending_ads_list_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $vw_pending_ads_list_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $vw_pending_ads_list_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $vw_pending_ads_list_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $vw_pending_ads_list_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($vw_pending_ads_list_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($vw_pending_ads_list_list->TotalRecs == 0 && $vw_pending_ads_list->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($vw_pending_ads_list_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fvw_pending_ads_listlistsrch.Init();
fvw_pending_ads_listlistsrch.FilterList = <?php echo $vw_pending_ads_list_list->GetFilterList() ?>;
fvw_pending_ads_listlist.Init();
</script>
<?php
$vw_pending_ads_list_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$vw_pending_ads_list_list->Page_Terminate();
?>
