...
$classified_pictures_edit = NULL; // Initialize page object first

class cclassified_pictures_edit extends cclassified_pictures {
	...
	// Page object name
	var $PageObjName = 'classified_pictures_edit';
	...
	function Page_Main() {
		...
		if ($this->ID->CurrentValue == "")
			$this->Page_Terminate("classified_pictureslist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("classified_pictureslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "classified_pictureslist.php")
	...
	function RenderRow() {
		...
		$this->classified_id->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->picture_link);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// picture_link
			if ($this->picture_link->Visible && !$this->picture_link->ReadOnly && !$this->picture_link->Upload->KeepFile) {
				$this->picture_link->Upload->DbValue = $rsold['picture_link']; // Get original value
...
// Create page object
if (!isset($classified_pictures_edit)) $classified_pictures_edit = new cclassified_pictures_edit();

// Page init
$classified_pictures_edit->Page_Init();

// Page main
$classified_pictures_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$classified_pictures_edit->Page_Render();
?>
...
var CurrentForm = fclassified_picturesedit = new ew_Form("fclassified_picturesedit", "edit");

// Validate form
fclassified_picturesedit.Validate = function() {
...
fclassified_picturesedit.Form_CustomValidate = 
...
fclassified_picturesedit.ValidateRequired = true;
<?php } else { ?>
fclassified_picturesedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fclassified_picturesedit.Lists["x_classified_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $classified_pictures_edit->ShowPageHeader(); ?>
<?php
$classified_pictures_edit->ShowMessage();
?>
<form name="fclassified_picturesedit" id="fclassified_picturesedit" class="<?php echo $classified_pictures_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($classified_pictures_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $classified_pictures_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="classified_pictures">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<?php if ($classified_pictures->getCurrentMasterTable() == "classified_data") { ?>
...
<?php if ($classified_pictures->picture_link->Visible) { // picture_link ?>
	<div id="r_picture_link" class="form-group">
		<label id="elh_classified_pictures_picture_link" class="col-sm-2 control-label ewLabel"><?php echo $classified_pictures->picture_link->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $classified_pictures->picture_link->CellAttributes() ?>>
...
<span title="<?php echo $classified_pictures->picture_link->FldTitle() ? $classified_pictures->picture_link->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($classified_pictures->picture_link->ReadOnly || $classified_pictures->picture_link->Disabled) echo " hide"; ?>">
	...
	<input type="file" title=" " data-table="classified_pictures" data-field="x_picture_link" name="x_picture_link" id="x_picture_link"<?php echo $classified_pictures->picture_link->EditAttributes() ?>>
</span>
<input type="hidden" name="fn_x_picture_link" id= "fn_x_picture_link" value="<?php echo $classified_pictures->picture_link->Upload->FileName ?>">
<?php if (@$_POST["fa_x_picture_link"] == "0") { ?>
<input type="hidden" name="fa_x_picture_link" id= "fa_x_picture_link" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_picture_link" id= "fa_x_picture_link" value="1">
<?php } ?>
<input type="hidden" name="fs_x_picture_link" id= "fs_x_picture_link" value="255">
<input type="hidden" name="fx_x_picture_link" id= "fx_x_picture_link" value="<?php echo $classified_pictures->picture_link->UploadAllowedFileExt ?>">
<input type="hidden" name="fm_x_picture_link" id= "fm_x_picture_link" value="<?php echo $classified_pictures->picture_link->UploadMaxFileSize ?>">
</div>
<table id="ft_x_picture_link" class="table table-condensed pull-left ewUploadTable"><tbody class="files"></tbody></table>
</span>
<?php echo $classified_pictures->picture_link->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($classified_pictures->classified_id->Visible) { // classified_id ?>
	<div id="r_classified_id" class="form-group">
		<label id="elh_classified_pictures_classified_id" class="col-sm-2 control-label ewLabel"><?php echo $classified_pictures->classified_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $classified_pictures->classified_id->CellAttributes() ?>>
...
<input type="hidden" id="x_classified_id" name="x_classified_id" value="<?php echo ew_HtmlEncode($classified_pictures->classified_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_classified_pictures_classified_id">
<?php
...
<script type="text/javascript">
fclassified_picturesedit.CreateAutoSuggest({"id":"x_classified_id","forceSelect":false});
</script>
</span>
<?php } ?>
<?php echo $classified_pictures->classified_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-table="classified_pictures" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($classified_pictures->ID->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $classified_pictures_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fclassified_picturesedit.Init();
</script>
<?php
$classified_pictures_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$classified_pictures_edit->Page_Terminate();
?>
