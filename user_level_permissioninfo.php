...
$user_level_permission = NULL;

//
// Table class for user_level_permission
//
class cuser_level_permission extends cTable {
	var $user_level_id;
	var $table_name;
	var $permission;
	...
	function __construct() {
		...
		// user_level_id
		$this->user_level_id = new cField('user_level_permission', 'user_level_permission', 'x_user_level_id', 'user_level_id', '`user_level_id`', '`user_level_id`', 3, -1, FALSE, '`user_level_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_level_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_level_id'] = &$this->user_level_id;

		// table_name
		$this->table_name = new cField('user_level_permission', 'user_level_permission', 'x_table_name', 'table_name', '`table_name`', '`table_name`', 200, -1, FALSE, '`table_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['table_name'] = &$this->table_name;

		// permission
		$this->permission = new cField('user_level_permission', 'user_level_permission', 'x_permission', 'permission', '`permission`', '`permission`', 3, -1, FALSE, '`permission`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`user_level_permission`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			...
			if (array_key_exists('table_name', $rs))
				ew_AddFilter($where, ew_QuotedName('table_name', $this->DBID) . '=' . ew_QuotedValue($rs['table_name'], $this->table_name->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@table_name@", ew_AdjustSql($this->table_name->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "user_level_permissionlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("user_level_permissionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("user_level_permissionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "user_level_permissionadd.php?" . $this->UrlParm($parm);
		else
			$url = "user_level_permissionadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("user_level_permissionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("user_level_permissionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("user_level_permissiondelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		...
		$json .= ",table_name:" . ew_VarToJson($this->table_name->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->table_name->CurrentValue)) {
			$sUrl .= "&table_name=" . urlencode($this->table_name->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->table_name->Exportable) $Doc->ExportCaption($this->table_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->table_name->Exportable) $Doc->ExportField($this->table_name);
