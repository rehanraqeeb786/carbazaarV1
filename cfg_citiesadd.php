...
$cfg_cities_add = NULL; // Initialize page object first

class ccfg_cities_add extends ccfg_cities {
	...
	// Page object name
	var $PageObjName = 'cfg_cities_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("cfg_citieslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "cfg_citieslist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "cfg_citiesview.php")
...
// Create page object
if (!isset($cfg_cities_add)) $cfg_cities_add = new ccfg_cities_add();

// Page init
$cfg_cities_add->Page_Init();

// Page main
$cfg_cities_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_cities_add->Page_Render();
?>
...
var CurrentForm = fcfg_citiesadd = new ew_Form("fcfg_citiesadd", "add");

// Validate form
fcfg_citiesadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $cfg_cities->city_name->FldCaption(), $cfg_cities->city_name->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_province_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $cfg_cities->province_id->FldCaption(), $cfg_cities->province_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_status");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $cfg_cities->status->FldCaption(), $cfg_cities->status->ReqErrMsg)) ?>");
...
fcfg_citiesadd.Form_CustomValidate = 
...
fcfg_citiesadd.ValidateRequired = true;
<?php } else { ?>
fcfg_citiesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_citiesadd.Lists["x_province_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_province_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_citiesadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_citiesadd.Lists["x_status"].Options = <?php echo json_encode($cfg_cities->status->Options()) ?>;
...
<?php $cfg_cities_add->ShowPageHeader(); ?>
<?php
$cfg_cities_add->ShowMessage();
?>
<form name="fcfg_citiesadd" id="fcfg_citiesadd" class="<?php echo $cfg_cities_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_cities_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_cities_add->Token ?>">
<?php } ?>
...
<?php if ($cfg_cities->city_name->Visible) { // city_name ?>
	<div id="r_city_name" class="form-group">
		<label id="elh_cfg_cities_city_name" for="x_city_name" class="col-sm-2 control-label ewLabel"><?php echo $cfg_cities->city_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $cfg_cities->city_name->CellAttributes() ?>>
...
<?php echo $cfg_cities->city_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cfg_cities->province_id->Visible) { // province_id ?>
	<div id="r_province_id" class="form-group">
		<label id="elh_cfg_cities_province_id" for="x_province_id" class="col-sm-2 control-label ewLabel"><?php echo $cfg_cities->province_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $cfg_cities->province_id->CellAttributes() ?>>
...
if (is_array($cfg_cities->province_id->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($cfg_cities->province_id->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
...
<?php echo $cfg_cities->province_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cfg_cities->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label id="elh_cfg_cities_status" class="col-sm-2 control-label ewLabel"><?php echo $cfg_cities->status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $cfg_cities->status->CellAttributes() ?>>
...
if (is_array($arwrk)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($cfg_cities->status->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked" : "";
		if ($selwrk <> "")
			$emptywrk = FALSE;
?>
...
<?php echo $cfg_cities->status->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_cities_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
