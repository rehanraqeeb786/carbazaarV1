...
$ad_pictures_delete = NULL; // Initialize page object first

class cad_pictures_delete extends cad_pictures {
	...
	// Page object name
	var $PageObjName = 'ad_pictures_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("ad_pictureslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in ad_pictures class, ad_picturesinfo.php
...
// Create page object
if (!isset($ad_pictures_delete)) $ad_pictures_delete = new cad_pictures_delete();

// Page init
$ad_pictures_delete->Page_Init();

// Page main
$ad_pictures_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ad_pictures_delete->Page_Render();
?>
...
var CurrentForm = fad_picturesdelete = new ew_Form("fad_picturesdelete", "delete");

// Form_CustomValidate event
fad_picturesdelete.Form_CustomValidate = 
...
fad_picturesdelete.ValidateRequired = true;
<?php } else { ?>
fad_picturesdelete.ValidateRequired = false; 
<?php } ?>
...
if ($ad_pictures_delete->Recordset = $ad_pictures_delete->LoadRecordset())
	$ad_pictures_deleteTotalRecs = $ad_pictures_delete->Recordset->RecordCount(); // Get record count
if ($ad_pictures_deleteTotalRecs <= 0) { // No record found, exit
	if ($ad_pictures_delete->Recordset)
		$ad_pictures_delete->Recordset->Close();
	$ad_pictures_delete->Page_Terminate("ad_pictureslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $ad_pictures_delete->ShowPageHeader(); ?>
<?php
$ad_pictures_delete->ShowMessage();
?>
<form name="fad_picturesdelete" id="fad_picturesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($ad_pictures_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $ad_pictures_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="ad_pictures">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($ad_pictures_delete->RecKeys as $key) { ?>
...
<?php if ($ad_pictures->ad_id->Visible) { // ad_id ?>
		<th><span id="elh_ad_pictures_ad_id" class="ad_pictures_ad_id"><?php echo $ad_pictures->ad_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($ad_pictures->picture_link->Visible) { // picture_link ?>
		<th><span id="elh_ad_pictures_picture_link" class="ad_pictures_picture_link"><?php echo $ad_pictures->picture_link->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$ad_pictures_delete->RecCnt = 0;
$i = 0;
while (!$ad_pictures_delete->Recordset->EOF) {
	$ad_pictures_delete->RecCnt++;
	$ad_pictures_delete->RowCnt++;

	// Set row properties
	$ad_pictures->ResetAttrs();
	$ad_pictures->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$ad_pictures_delete->LoadRowValues($ad_pictures_delete->Recordset);

	// Render row
	$ad_pictures_delete->RenderRow();
?>
	<tr<?php echo $ad_pictures->RowAttributes() ?>>
<?php if ($ad_pictures->ad_id->Visible) { // ad_id ?>
		<td<?php echo $ad_pictures->ad_id->CellAttributes() ?>>
<span id="el<?php echo $ad_pictures_delete->RowCnt ?>_ad_pictures_ad_id" class="ad_pictures_ad_id">
...
<?php } ?>
<?php if ($ad_pictures->picture_link->Visible) { // picture_link ?>
		<td<?php echo $ad_pictures->picture_link->CellAttributes() ?>>
<span id="el<?php echo $ad_pictures_delete->RowCnt ?>_ad_pictures_picture_link" class="ad_pictures_picture_link">
...
<?php
	$ad_pictures_delete->Recordset->MoveNext();
}
$ad_pictures_delete->Recordset->Close();
?>
</tbody>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $ad_pictures_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fad_picturesdelete.Init();
</script>
<?php
$ad_pictures_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ad_pictures_delete->Page_Terminate();
?>
