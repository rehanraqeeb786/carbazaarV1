...
$classified_colors = NULL;

//
// Table class for classified_colors
//
class cclassified_colors extends cTable {
	var $ID;
	var $color_id;
	var $classfied_id;
	var $ETD;
	...
	function __construct() {
		...
		$this->TableVar = 'classified_colors';
		$this->TableName = 'classified_colors';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`classified_colors`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new cField('classified_colors', 'classified_colors', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// color_id
		$this->color_id = new cField('classified_colors', 'classified_colors', 'x_color_id', 'color_id', '`color_id`', '`color_id`', 3, -1, FALSE, '`color_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->color_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['color_id'] = &$this->color_id;

		// classfied_id
		$this->classfied_id = new cField('classified_colors', 'classified_colors', 'x_classfied_id', 'classfied_id', '`classfied_id`', '`classfied_id`', 3, -1, FALSE, '`classfied_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->classfied_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['classfied_id'] = &$this->classfied_id;

		// ETD
		$this->ETD = new cField('classified_colors', 'classified_colors', 'x_ETD', 'ETD', '`ETD`', 'DATE_FORMAT(`ETD`, \'%Y/%m/%d\')', 135, 5, FALSE, '`ETD`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "classified_data") {
			if ($this->classfied_id->getSessionValue() <> "")
				$sMasterFilter .= "`ID`=" . ew_QuotedValue($this->classfied_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "classified_data") {
			if ($this->classfied_id->getSessionValue() <> "")
				$sDetailFilter .= "`classfied_id`=" . ew_QuotedValue($this->classfied_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function SqlDetailFilter_classified_data() {
		return "`classfied_id`=@classfied_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`classified_colors`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "classified_colorslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("classified_colorsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("classified_colorsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "classified_colorsadd.php?" . $this->UrlParm($parm);
		else
			$url = "classified_colorsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("classified_colorsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("classified_colorsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("classified_colorsdelete.php", $this->UrlParm());
	}
	...
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "classified_data" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			...
			$url .= "&fk_ID=" . urlencode($this->classfied_id->CurrentValue);
		}
		return $url;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->color_id->EditValue = $this->color_id->CurrentValue;
		...
		$this->classfied_id->EditValue = $this->classfied_id->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->classfied_id->Exportable) $Doc->ExportCaption($this->classfied_id);
				} else {
					if ($this->color_id->Exportable) $Doc->ExportCaption($this->color_id);
					if ($this->classfied_id->Exportable) $Doc->ExportCaption($this->classfied_id);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->classfied_id->Exportable) $Doc->ExportField($this->classfied_id);
					} else {
						if ($this->color_id->Exportable) $Doc->ExportField($this->color_id);
						if ($this->classfied_id->Exportable) $Doc->ExportField($this->classfied_id);
					}
