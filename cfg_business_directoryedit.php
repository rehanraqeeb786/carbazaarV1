...
$cfg_business_directory_edit = NULL; // Initialize page object first

class ccfg_business_directory_edit extends ccfg_business_directory {
	...
	// Page object name
	var $PageObjName = 'cfg_business_directory_edit';
	...
	function Page_Main() {
		...
		if ($this->ID->CurrentValue == "")
			$this->Page_Terminate("cfg_business_directorylist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->business_logo_link);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->image_2);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->img_3);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// business_title
			$this->business_title->SetDbValueDef($rsnew, $this->business_title->CurrentValue, "", $this->business_title->ReadOnly);
			...
			// business_address
			$this->business_address->SetDbValueDef($rsnew, $this->business_address->CurrentValue, "", $this->business_address->ReadOnly);

			// business_logo_link
			if ($this->business_logo_link->Visible && !$this->business_logo_link->ReadOnly && !$this->business_logo_link->Upload->KeepFile) {
				$this->business_logo_link->Upload->DbValue = $rsold['business_logo_link']; // Get original value
			...
			// image_2
			if ($this->image_2->Visible && !$this->image_2->ReadOnly && !$this->image_2->Upload->KeepFile) {
				$this->image_2->Upload->DbValue = $rsold['image_2']; // Get original value
			...
			// img_3
			if ($this->img_3->Visible && !$this->img_3->ReadOnly && !$this->img_3->Upload->KeepFile) {
				$this->img_3->Upload->DbValue = $rsold['img_3']; // Get original value
			...
			// detail_desc
			$this->detail_desc->SetDbValueDef($rsnew, $this->detail_desc->CurrentValue, NULL, $this->detail_desc->ReadOnly);

			// longitute
			$this->longitute->SetDbValueDef($rsnew, $this->longitute->CurrentValue, NULL, $this->longitute->ReadOnly);

			// latitude
			$this->latitude->SetDbValueDef($rsnew, $this->latitude->CurrentValue, NULL, $this->latitude->ReadOnly);

			// primary_number
			$this->primary_number->SetDbValueDef($rsnew, $this->primary_number->CurrentValue, 0, $this->primary_number->ReadOnly);

			// secondary_number
			$this->secondary_number->SetDbValueDef($rsnew, $this->secondary_number->CurrentValue, NULL, $this->secondary_number->ReadOnly);

			// fb_page
			$this->fb_page->SetDbValueDef($rsnew, $this->fb_page->CurrentValue, NULL, $this->fb_page->ReadOnly);

			// timings
			$this->timings->SetDbValueDef($rsnew, $this->timings->CurrentValue, NULL, $this->timings->ReadOnly);
...
// Create page object
if (!isset($cfg_business_directory_edit)) $cfg_business_directory_edit = new ccfg_business_directory_edit();

// Page init
$cfg_business_directory_edit->Page_Init();

// Page main
$cfg_business_directory_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_business_directory_edit->Page_Render();
?>
...
var CurrentForm = fcfg_business_directoryedit = new ew_Form("fcfg_business_directoryedit", "edit");

// Validate form
fcfg_business_directoryedit.Validate = function() {
...
fcfg_business_directoryedit.Form_CustomValidate = 
...
fcfg_business_directoryedit.ValidateRequired = true;
<?php } else { ?>
fcfg_business_directoryedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_business_directoryedit.Lists["x_cat_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_cat_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_business_directoryedit.Lists["x_province_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_province_name","","",""],"ParentFields":[],"ChildFields":["x_city_id"],"FilterFields":[],"Options":[],"Template":""};
fcfg_business_directoryedit.Lists["x_city_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_city_name","","",""],"ParentFields":["x_province_id"],"ChildFields":[],"FilterFields":["x_province_id"],"Options":[],"Template":""};
fcfg_business_directoryedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_business_directoryedit.Lists["x_status"].Options = <?php echo json_encode($cfg_business_directory->status->Options()) ?>;
...
<?php $cfg_business_directory_edit->ShowPageHeader(); ?>
<?php
$cfg_business_directory_edit->ShowMessage();
?>
<form name="fcfg_business_directoryedit" id="fcfg_business_directoryedit" class="<?php echo $cfg_business_directory_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_business_directory_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_business_directory_edit->Token ?>">
<?php } ?>
...
<?php if (@$_POST["fa_x_business_logo_link"] == "0") { ?>
<input type="hidden" name="fa_x_business_logo_link" id= "fa_x_business_logo_link" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_business_logo_link" id= "fa_x_business_logo_link" value="1">
<?php } ?>
...
<?php if (@$_POST["fa_x_image_2"] == "0") { ?>
<input type="hidden" name="fa_x_image_2" id= "fa_x_image_2" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_image_2" id= "fa_x_image_2" value="1">
<?php } ?>
...
<?php if (@$_POST["fa_x_img_3"] == "0") { ?>
<input type="hidden" name="fa_x_img_3" id= "fa_x_img_3" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_img_3" id= "fa_x_img_3" value="1">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fcfg_business_directoryedit", "x_detail_desc", 35, 4, <?php echo ($cfg_business_directory->detail_desc->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<input type="hidden" data-table="cfg_business_directory" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($cfg_business_directory->ID->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_business_directory_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fcfg_business_directoryedit.Init();
</script>
<?php
$cfg_business_directory_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_business_directory_edit->Page_Terminate();
?>
