...
//
// Table class for cfg_features
//
class ccfg_features extends cTable {
	var $ID;
	var $feature_name;
	var $feature_icon_link;
	var $details;
	var $status;
	var $created_at;
	var $updated_at;
	...
	function __construct() {
		...
		$this->TableVar = 'cfg_features';
		$this->TableName = 'cfg_features';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cfg_features`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new cField('cfg_features', 'cfg_features', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// feature_name
		$this->feature_name = new cField('cfg_features', 'cfg_features', 'x_feature_name', 'feature_name', '`feature_name`', '`feature_name`', 200, -1, FALSE, '`feature_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['feature_name'] = &$this->feature_name;

		// feature_icon_link
		$this->feature_icon_link = new cField('cfg_features', 'cfg_features', 'x_feature_icon_link', 'feature_icon_link', '`feature_icon_link`', '`feature_icon_link`', 200, -1, TRUE, '`feature_icon_link`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->fields['feature_icon_link'] = &$this->feature_icon_link;

		// details
		$this->details = new cField('cfg_features', 'cfg_features', 'x_details', 'details', '`details`', '`details`', 201, -1, FALSE, '`details`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['details'] = &$this->details;

		// status
		$this->status = new cField('cfg_features', 'cfg_features', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->status->OptionCount = 2;
		$this->fields['status'] = &$this->status;

		// created_at
		$this->created_at = new cField('cfg_features', 'cfg_features', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('cfg_features', 'cfg_features', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cfg_features`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cfg_featuresview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cfg_featuresview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cfg_featuresadd.php?" . $this->UrlParm($parm);
		else
			$url = "cfg_featuresadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("cfg_featuresedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("cfg_featuresadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cfg_featuresdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->feature_name->EditValue = $this->feature_name->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->feature_name->Exportable) $Doc->ExportCaption($this->feature_name);
					if ($this->feature_icon_link->Exportable) $Doc->ExportCaption($this->feature_icon_link);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->feature_name->Exportable) $Doc->ExportField($this->feature_name);
						if ($this->feature_icon_link->Exportable) $Doc->ExportField($this->feature_icon_link);
