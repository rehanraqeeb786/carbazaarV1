...
//
// Table class for packages
//
class cpackages extends cTable {
	var $id;
	var $number_of_days;
	var $package_fee;
	...
	function __construct() {
		...
		$this->TableVar = 'packages';
		$this->TableName = 'packages';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`packages`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('packages', 'packages', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// number_of_days
		$this->number_of_days = new cField('packages', 'packages', 'x_number_of_days', 'number_of_days', '`number_of_days`', '`number_of_days`', 3, -1, FALSE, '`number_of_days`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->number_of_days->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['number_of_days'] = &$this->number_of_days;

		// package_fee
		$this->package_fee = new cField('packages', 'packages', 'x_package_fee', 'package_fee', '`package_fee`', '`package_fee`', 3, -1, FALSE, '`package_fee`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->package_fee->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['package_fee'] = &$this->package_fee;

		// short_description
		$this->short_description = new cField('packages', 'packages', 'x_short_description', 'short_description', '`short_description`', '`short_description`', 200, -1, FALSE, '`short_description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['short_description'] = &$this->short_description;

		// status
		$this->status = new cField('packages', 'packages', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->status->OptionCount = 2;
		$this->fields['status'] = &$this->status;

		// created_at
		$this->created_at = new cField('packages', 'packages', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('packages', 'packages', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`packages`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("packagesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("packagesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "packagesadd.php?" . $this->UrlParm($parm);
		else
			$url = "packagesadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("packagesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("packagesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("packagesdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->package_fee->EditValue = $this->package_fee->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->number_of_days->Exportable) $Doc->ExportCaption($this->number_of_days);
					if ($this->package_fee->Exportable) $Doc->ExportCaption($this->package_fee);
					if ($this->short_description->Exportable) $Doc->ExportCaption($this->short_description);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->number_of_days->Exportable) $Doc->ExportField($this->number_of_days);
						if ($this->package_fee->Exportable) $Doc->ExportField($this->package_fee);
						if ($this->short_description->Exportable) $Doc->ExportField($this->short_description);
