...
$cfg_make_companies = NULL;

//
// Table class for cfg_make_companies
//
class ccfg_make_companies extends cTable {
	var $ID;
	var $title;
	var $logo_link;
	var $details;
	...
	function __construct() {
		...
		$this->TableVar = 'cfg_make_companies';
		$this->TableName = 'cfg_make_companies';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cfg_make_companies`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// ID
		$this->ID = new cField('cfg_make_companies', 'cfg_make_companies', 'x_ID', 'ID', '`ID`', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] = &$this->ID;

		// title
		$this->title = new cField('cfg_make_companies', 'cfg_make_companies', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['title'] = &$this->title;

		// logo_link
		$this->logo_link = new cField('cfg_make_companies', 'cfg_make_companies', 'x_logo_link', 'logo_link', '`logo_link`', '`logo_link`', 200, -1, TRUE, '`logo_link`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->fields['logo_link'] = &$this->logo_link;

		// details
		$this->details = new cField('cfg_make_companies', 'cfg_make_companies', 'x_details', 'details', '`details`', '`details`', 201, -1, FALSE, '`details`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['details'] = &$this->details;

		// status
		$this->status = new cField('cfg_make_companies', 'cfg_make_companies', 'x_status', 'status', '`status`', '`status`', 202, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->status->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->status->OptionCount = 2;
		$this->fields['status'] = &$this->status;

		// created_at
		$this->created_at = new cField('cfg_make_companies', 'cfg_make_companies', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('cfg_make_companies', 'cfg_make_companies', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cfg_make_companies`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "cfg_make_companieslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cfg_make_companiesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cfg_make_companiesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cfg_make_companiesadd.php?" . $this->UrlParm($parm);
		else
			$url = "cfg_make_companiesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("cfg_make_companiesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("cfg_make_companiesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cfg_make_companiesdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->logo_link->Exportable) $Doc->ExportCaption($this->logo_link);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->logo_link->Exportable) $Doc->ExportField($this->logo_link);
