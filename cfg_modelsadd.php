...
$cfg_models_add = NULL; // Initialize page object first

class ccfg_models_add extends ccfg_models {
	...
	// Page object name
	var $PageObjName = 'cfg_models_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "cfg_modelsview.php")
	...
	function LoadDefaultValues() {
		...
		$this->make_company_id->CurrentValue = NULL;
		$this->make_company_id->OldValue = $this->make_company_id->CurrentValue;
		$this->main_pic_link->Upload->DbValue = NULL;
		$this->main_pic_link->OldValue = $this->main_pic_link->Upload->DbValue;
		$this->main_pic_link->CurrentValue = NULL; // Clear file related field
		$this->pic_2->Upload->DbValue = NULL;
		$this->pic_2->OldValue = $this->pic_2->Upload->DbValue;
		$this->pic_2->CurrentValue = NULL; // Clear file related field
		$this->pic_3->Upload->DbValue = NULL;
		$this->pic_3->OldValue = $this->pic_3->Upload->DbValue;
		$this->pic_3->CurrentValue = NULL; // Clear file related field
		$this->pic_4->Upload->DbValue = NULL;
		$this->pic_4->OldValue = $this->pic_4->Upload->DbValue;
		$this->pic_4->CurrentValue = NULL; // Clear file related field
		$this->pic_5->Upload->DbValue = NULL;
		$this->pic_5->OldValue = $this->pic_5->Upload->DbValue;
		$this->pic_5->CurrentValue = NULL; // Clear file related field
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->main_pic_link);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->pic_2);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->pic_3);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->pic_4);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->pic_5);
	...
	function AddRow($rsold = NULL) {
		...
		// make_company_id
		$this->make_company_id->SetDbValueDef($rsnew, $this->make_company_id->CurrentValue, 0, FALSE);

		// main_pic_link
		if ($this->main_pic_link->Visible && !$this->main_pic_link->Upload->KeepFile) {
			$this->main_pic_link->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->pic_2->Visible && !$this->pic_2->Upload->KeepFile) {
			$this->pic_2->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->pic_3->Visible && !$this->pic_3->Upload->KeepFile) {
			$this->pic_3->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->pic_4->Visible && !$this->pic_4->Upload->KeepFile) {
			$this->pic_4->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->pic_5->Visible && !$this->pic_5->Upload->KeepFile) {
			$this->pic_5->Upload->DbValue = ""; // No need to delete old file
...
// Create page object
if (!isset($cfg_models_add)) $cfg_models_add = new ccfg_models_add();

// Page init
$cfg_models_add->Page_Init();

// Page main
$cfg_models_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_models_add->Page_Render();
?>
...
var CurrentForm = fcfg_modelsadd = new ew_Form("fcfg_modelsadd", "add");

// Validate form
fcfg_modelsadd.Validate = function() {
...
fcfg_modelsadd.Form_CustomValidate = 
...
fcfg_modelsadd.ValidateRequired = true;
<?php } else { ?>
fcfg_modelsadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_modelsadd.Lists["x_make_company_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_modelsadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_modelsadd.Lists["x_status"].Options = <?php echo json_encode($cfg_models->status->Options()) ?>;
...
<?php $cfg_models_add->ShowPageHeader(); ?>
<?php
$cfg_models_add->ShowMessage();
?>
<form name="fcfg_modelsadd" id="fcfg_modelsadd" class="<?php echo $cfg_models_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_models_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_models_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fcfg_modelsadd", "x_details", 35, 4, <?php echo ($cfg_models->details->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_models_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
