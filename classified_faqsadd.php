...
$classified_faqs_add = NULL; // Initialize page object first

class cclassified_faqs_add extends cclassified_faqs {
	...
	// Page object name
	var $PageObjName = 'classified_faqs_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("classified_faqslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "classified_faqslist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "classified_faqsview.php")
	...
	function LoadDefaultValues() {
		$this->classified_id->CurrentValue = NULL;
		$this->classified_id->OldValue = $this->classified_id->CurrentValue;
		$this->question_text->CurrentValue = NULL;
		$this->question_text->OldValue = $this->question_text->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->question_text->FldIsDetailKey) {
			$this->question_text->setFormValue($objForm->GetValue("x_question_text"));
		}
	...
	function RenderRow() {
		...
		$this->answer_text->ViewCustomAttributes = "";
			...
			$this->question_text->EditValue = ew_HtmlEncode($this->question_text->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!$this->question_text->FldIsDetailKey && !is_null($this->question_text->FormValue) && $this->question_text->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->question_text->FldCaption(), $this->question_text->ReqErrMsg));
		}
	...
	function AddRow($rsold = NULL) {
		...
		// classified_id
		$this->classified_id->SetDbValueDef($rsnew, $this->classified_id->CurrentValue, 0, FALSE);

		// question_text
		$this->question_text->SetDbValueDef($rsnew, $this->question_text->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($classified_faqs_add)) $classified_faqs_add = new cclassified_faqs_add();

// Page init
$classified_faqs_add->Page_Init();

// Page main
$classified_faqs_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$classified_faqs_add->Page_Render();
?>
...
var CurrentForm = fclassified_faqsadd = new ew_Form("fclassified_faqsadd", "add");

// Validate form
fclassified_faqsadd.Validate = function() {
...
fclassified_faqsadd.Form_CustomValidate = 
...
fclassified_faqsadd.ValidateRequired = true;
<?php } else { ?>
fclassified_faqsadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fclassified_faqsadd.Lists["x_classified_id"] = {"LinkField":"x_ID","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $classified_faqs_add->ShowPageHeader(); ?>
<?php
$classified_faqs_add->ShowMessage();
?>
<form name="fclassified_faqsadd" id="fclassified_faqsadd" class="<?php echo $classified_faqs_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($classified_faqs_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $classified_faqs_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="classified_faqs">
<input type="hidden" name="a_add" id="a_add" value="A">
<?php if ($classified_faqs->getCurrentMasterTable() == "classified_data") { ?>
...
<?php if ($classified_faqs->classified_id->Visible) { // classified_id ?>
	<div id="r_classified_id" class="form-group">
		<label id="elh_classified_faqs_classified_id" class="col-sm-2 control-label ewLabel"><?php echo $classified_faqs->classified_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $classified_faqs->classified_id->CellAttributes() ?>>
...
<input type="hidden" id="x_classified_id" name="x_classified_id" value="<?php echo ew_HtmlEncode($classified_faqs->classified_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_classified_faqs_classified_id">
<?php
...
<input type="hidden" data-table="classified_faqs" data-field="x_classified_id" data-value-separator="<?php echo ew_HtmlEncode(is_array($classified_faqs->classified_id->DisplayValueSeparator) ? json_encode($classified_faqs->classified_id->DisplayValueSeparator) : $classified_faqs->classified_id->DisplayValueSeparator) ?>" name="x_classified_id" id="x_classified_id" value="<?php echo ew_HtmlEncode($classified_faqs->classified_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<?php
...
<script type="text/javascript">
fclassified_faqsadd.CreateAutoSuggest({"id":"x_classified_id","forceSelect":false});
</script>
</span>
<?php } ?>
<?php echo $classified_faqs->classified_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($classified_faqs->question_text->Visible) { // question_text ?>
	<div id="r_question_text" class="form-group">
		<label id="elh_classified_faqs_question_text" for="x_question_text" class="col-sm-2 control-label ewLabel"><?php echo $classified_faqs->question_text->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $classified_faqs->question_text->CellAttributes() ?>>
<span id="el_classified_faqs_question_text">
<textarea data-table="classified_faqs" data-field="x_question_text" name="x_question_text" id="x_question_text" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($classified_faqs->question_text->getPlaceHolder()) ?>"<?php echo $classified_faqs->question_text->EditAttributes() ?>><?php echo $classified_faqs->question_text->EditValue ?></textarea>
</span>
<?php echo $classified_faqs->question_text->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($classified_faqs->answer_text->Visible) { // answer_text ?>
	<div id="r_answer_text" class="form-group">
		<label id="elh_classified_faqs_answer_text" for="x_answer_text" class="col-sm-2 control-label ewLabel"><?php echo $classified_faqs->answer_text->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $classified_faqs->answer_text->CellAttributes() ?>>
<span id="el_classified_faqs_answer_text">
<textarea data-table="classified_faqs" data-field="x_answer_text" name="x_answer_text" id="x_answer_text" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($classified_faqs->answer_text->getPlaceHolder()) ?>"<?php echo $classified_faqs->answer_text->EditAttributes() ?>><?php echo $classified_faqs->answer_text->EditValue ?></textarea>
</span>
<?php echo $classified_faqs->answer_text->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $classified_faqs_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fclassified_faqsadd.Init();
</script>
<?php
$classified_faqs_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$classified_faqs_add->Page_Terminate();
?>
