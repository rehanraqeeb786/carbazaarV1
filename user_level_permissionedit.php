...
$user_level_permission_edit = NULL; // Initialize page object first

class cuser_level_permission_edit extends cuser_level_permission {
	...
	// Page object name
	var $PageObjName = 'user_level_permission_edit';
	...
	function Page_Main() {
		...
		if ($this->user_level_id->CurrentValue == "")
			$this->Page_Terminate("user_level_permissionlist.php"); // Invalid key, return to list
		if ($this->table_name->CurrentValue == "")
			$this->Page_Terminate("user_level_permissionlist.php"); // Invalid key, return to list
...
// Create page object
if (!isset($user_level_permission_edit)) $user_level_permission_edit = new cuser_level_permission_edit();

// Page init
$user_level_permission_edit->Page_Init();

// Page main
$user_level_permission_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$user_level_permission_edit->Page_Render();
?>
...
var CurrentForm = fuser_level_permissionedit = new ew_Form("fuser_level_permissionedit", "edit");
...
fuser_level_permissionedit.Form_CustomValidate = 
...
fuser_level_permissionedit.ValidateRequired = true;
<?php } else { ?>
fuser_level_permissionedit.ValidateRequired = false; 
<?php } ?>
...
<?php $user_level_permission_edit->ShowPageHeader(); ?>
<?php
$user_level_permission_edit->ShowMessage();
?>
<form name="fuser_level_permissionedit" id="fuser_level_permissionedit" class="<?php echo $user_level_permission_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($user_level_permission_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $user_level_permission_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $user_level_permission->user_level_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="user_level_permission" data-field="x_user_level_id" name="x_user_level_id" id="x_user_level_id" value="<?php echo ew_HtmlEncode($user_level_permission->user_level_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $user_level_permission->table_name->EditValue ?></p></span>
</span>
<input type="hidden" data-table="user_level_permission" data-field="x_table_name" name="x_table_name" id="x_table_name" value="<?php echo ew_HtmlEncode($user_level_permission->table_name->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $user_level_permission_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fuser_level_permissionedit.Init();
</script>
<?php
$user_level_permission_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$user_level_permission_edit->Page_Terminate();
?>
