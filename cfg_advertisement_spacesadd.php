...
$cfg_advertisement_spaces_add = NULL; // Initialize page object first

class ccfg_advertisement_spaces_add extends ccfg_advertisement_spaces {
	...
	// Page object name
	var $PageObjName = 'cfg_advertisement_spaces_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "cfg_advertisement_spacesview.php")
	...
	function LoadDefaultValues() {
		$this->ad_space_name->CurrentValue = NULL;
		$this->ad_space_name->OldValue = $this->ad_space_name->CurrentValue;
		$this->ad_type->CurrentValue = "Image";
		$this->ad_placement->CurrentValue = NULL;
		$this->ad_placement->OldValue = $this->ad_placement->CurrentValue;
		$this->ad_script->CurrentValue = NULL;
		$this->ad_script->OldValue = $this->ad_script->CurrentValue;
		$this->ad_image->Upload->DbValue = NULL;
		$this->ad_image->OldValue = $this->ad_image->Upload->DbValue;
		$this->ad_image->CurrentValue = NULL; // Clear file related field
		$this->link_for_image->CurrentValue = NULL;
		$this->link_for_image->OldValue = $this->link_for_image->CurrentValue;
		$this->ad_from->CurrentValue = NULL;
		$this->ad_from->OldValue = $this->ad_from->CurrentValue;
		$this->ad_till->CurrentValue = NULL;
		$this->ad_till->OldValue = $this->ad_till->CurrentValue;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->ad_image);
	...
	function AddRow($rsold = NULL) {
		...
		// ad_space_name
		$this->ad_space_name->SetDbValueDef($rsnew, $this->ad_space_name->CurrentValue, "", FALSE);

		// ad_type
		$this->ad_type->SetDbValueDef($rsnew, $this->ad_type->CurrentValue, "", strval($this->ad_type->CurrentValue) == "");

		// ad_placement
		$this->ad_placement->SetDbValueDef($rsnew, $this->ad_placement->CurrentValue, "", FALSE);

		// ad_script
		$this->ad_script->SetDbValueDef($rsnew, $this->ad_script->CurrentValue, NULL, FALSE);

		// ad_image
		if ($this->ad_image->Visible && !$this->ad_image->Upload->KeepFile) {
			$this->ad_image->Upload->DbValue = ""; // No need to delete old file
		...
		// link_for_image
		$this->link_for_image->SetDbValueDef($rsnew, $this->link_for_image->CurrentValue, "", FALSE);

		// ad_from
		$this->ad_from->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ad_from->CurrentValue, 5), ew_CurrentDate(), FALSE);

		// ad_till
		$this->ad_till->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->ad_till->CurrentValue, 5), ew_CurrentDate(), FALSE);
...
// Create page object
if (!isset($cfg_advertisement_spaces_add)) $cfg_advertisement_spaces_add = new ccfg_advertisement_spaces_add();

// Page init
$cfg_advertisement_spaces_add->Page_Init();

// Page main
$cfg_advertisement_spaces_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_advertisement_spaces_add->Page_Render();
?>
...
var CurrentForm = fcfg_advertisement_spacesadd = new ew_Form("fcfg_advertisement_spacesadd", "add");

// Validate form
fcfg_advertisement_spacesadd.Validate = function() {
...
fcfg_advertisement_spacesadd.Form_CustomValidate = 
...
fcfg_advertisement_spacesadd.ValidateRequired = true;
<?php } else { ?>
fcfg_advertisement_spacesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_advertisement_spacesadd.Lists["x_ad_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_advertisement_spacesadd.Lists["x_ad_type"].Options = <?php echo json_encode($cfg_advertisement_spaces->ad_type->Options()) ?>;
fcfg_advertisement_spacesadd.Lists["x_ad_placement"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_advertisement_spacesadd.Lists["x_ad_placement"].Options = <?php echo json_encode($cfg_advertisement_spaces->ad_placement->Options()) ?>;
fcfg_advertisement_spacesadd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_advertisement_spacesadd.Lists["x_status"].Options = <?php echo json_encode($cfg_advertisement_spaces->status->Options()) ?>;
...
<?php $cfg_advertisement_spaces_add->ShowPageHeader(); ?>
<?php
$cfg_advertisement_spaces_add->ShowMessage();
?>
<form name="fcfg_advertisement_spacesadd" id="fcfg_advertisement_spacesadd" class="<?php echo $cfg_advertisement_spaces_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_advertisement_spaces_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_advertisement_spaces_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fcfg_advertisement_spacesadd", "x_ad_from", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fcfg_advertisement_spacesadd", "x_ad_till", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_advertisement_spaces_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fcfg_advertisement_spacesadd.Init();
</script>
<?php
$cfg_advertisement_spaces_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_advertisement_spaces_add->Page_Terminate();
?>
