...
$cfg_body_type_edit = NULL; // Initialize page object first

class ccfg_body_type_edit extends ccfg_body_type {
	...
	// Page object name
	var $PageObjName = 'cfg_body_type_edit';
	...
	function Page_Main() {
		...
		if ($this->ID->CurrentValue == "")
			$this->Page_Terminate("cfg_body_typelist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->icon_url);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// icon_url
			if ($this->icon_url->Visible && !$this->icon_url->ReadOnly && !$this->icon_url->Upload->KeepFile) {
				$this->icon_url->Upload->DbValue = $rsold['icon_url']; // Get original value
...
// Create page object
if (!isset($cfg_body_type_edit)) $cfg_body_type_edit = new ccfg_body_type_edit();

// Page init
$cfg_body_type_edit->Page_Init();

// Page main
$cfg_body_type_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cfg_body_type_edit->Page_Render();
?>
...
var CurrentForm = fcfg_body_typeedit = new ew_Form("fcfg_body_typeedit", "edit");

// Validate form
fcfg_body_typeedit.Validate = function() {
...
fcfg_body_typeedit.Form_CustomValidate = 
...
fcfg_body_typeedit.ValidateRequired = true;
<?php } else { ?>
fcfg_body_typeedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcfg_body_typeedit.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcfg_body_typeedit.Lists["x_status"].Options = <?php echo json_encode($cfg_body_type->status->Options()) ?>;
...
<?php $cfg_body_type_edit->ShowPageHeader(); ?>
<?php
$cfg_body_type_edit->ShowMessage();
?>
<form name="fcfg_body_typeedit" id="fcfg_body_typeedit" class="<?php echo $cfg_body_type_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cfg_body_type_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cfg_body_type_edit->Token ?>">
<?php } ?>
...
<?php if (@$_POST["fa_x_icon_url"] == "0") { ?>
<input type="hidden" name="fa_x_icon_url" id= "fa_x_icon_url" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_icon_url" id= "fa_x_icon_url" value="1">
<?php } ?>
...
<input type="hidden" data-table="cfg_body_type" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo ew_HtmlEncode($cfg_body_type->ID->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cfg_body_type_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fcfg_body_typeedit.Init();
</script>
<?php
$cfg_body_type_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cfg_body_type_edit->Page_Terminate();
?>
